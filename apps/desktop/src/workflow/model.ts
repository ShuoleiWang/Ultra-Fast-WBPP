...
export const initialStages = (scope?: "panel" | "project"): StageProgress[] => STAGE_DEFINITIONS
...
export const DEMO_ARTIFACTS: OutputArtifact[] = [{ kind: "PREVIEW", name: "DEMO_result.png", path: "/explicit-browser-demo/result.png", detail: "DEMO ONLY · no file was created" }];
export const deduplicate = (values: string[]) => [...new Set(values)];
export const known = (value: string | undefined | null) => Boolean(value?.trim() && !["UNKNOWN", "UNSPECIFIED"].includes(value.trim().toUpperCase()));
...
export const unknownCfa = (value: string | undefined | null) => !value?.trim() || ["UNKNOWN", "UNSPECIFIED"].includes(value.trim().toUpperCase());
...
export const safeSourceId = (role: FrameRole, index: number) => `${role.toLowerCase().replaceAll("_", "-")}-${String(index + 1).padStart(4, "0")}`;
...
export function panelMatrix(assets: InspectedAsset[], admittedPaths: ReadonlySet<string> = new Set()): Array<PanelCell & { admittedCount: number }> {
...
export interface BlinkChannelSummary extends BlinkChannel { frames: BlinkFrame[]; total: number; viewed: number; confirmed: boolean; kept: number; flagged: number; exclude: number; attention: number; }
...
export function blinkSessionCovers(session: BlinkSession, lightPaths: string[]): boolean {
  if (inventoryKeyOf(lightPaths) !== session.inventoryKey || !lightPaths.length || !CONTENT_DIGEST.test(session.manifest.manifestSha256 ?? "")) return false;
