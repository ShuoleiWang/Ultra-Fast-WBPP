...
export function PreviewCanvas({ src, label, width, height, imageWidth, imageHeight, view, onPaint, onError, className = "", decoderId, pixelated = false }: {
  ...
  useEffect(() => {
    ...
    const visible = () => { if (!document.hidden) setDecoded((value) => value + 1); };
  ...
  useLayoutEffect(() => {
    ...
    if (!context) { callbacks.current.onError(); return; }
    ...
    const iw = imageWidth || image.naturalWidth, ih = imageHeight || image.naturalHeight;
    const scale = Math.min(width / iw, height / ih);
    const pose = view ?? { scale, x: (width - iw * scale) / 2, y: (height - ih * scale) / 2 };
    context.translate(pose.x, pose.y);
    context.scale(pose.scale, pose.scale);
    try { context.drawImage(image, 0, 0, iw, ih); }
