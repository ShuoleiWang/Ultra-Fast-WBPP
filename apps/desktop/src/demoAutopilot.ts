...
export function requestedDemoStage(): DemoStage | undefined {
  ...
  return value === "frames" || value === "review" || value === "blink" || value === "run" || value === "result"
    ? value
    : undefined;
}
...
export function useDemoAutopilot(workflow: Workflow, stage: DemoStage | undefined) {
  ...
  useEffect(() => {
    if (!stage) return;
    if (phase.current === 0 && workflow.importedTotal === 0) {
      phase.current = 1;
      workflow.loadDemo();
      return;
    }
    if (phase.current === 1 && workflow.sources.some((source) => source.detected && !source.confirmed)) {
      ...
      for (const source of workflow.sources)
        if (source.detected && !source.confirmed) workflow.confirmRole(source.role);
      return;
    }
    if (phase.current === 2 && stage === "blink" && workflow.allRequiredConfirmed && workflow.step === "import") {
      phase.current = 3;
      void workflow.runBlink();
      return;
    }
    if (phase.current === 2 && stage !== "frames" && workflow.allRequiredConfirmed && workflow.step === "import") {
      phase.current = 3;
      void workflow.runInspection();
      return;
    }
    if (
      phase.current === 3 &&
...
const flag = (
  code: string,
  severity: BlinkFlag["severity"],
  value: number | null,
  threshold: number | null,
  message: string,
  combined = false,
): BlinkFlag => ({ code, severity, value, threshold, combined, message });
...
function starField(seed: number, count: number) {
  ...
  return Array.from({ length: count }, () => {
    const bright = next();
    return { x: 8 + next() * 375, y: 8 + next() * 245, r: 0.5 + bright * bright * 2.2, o: 0.35 + bright * 0.65 };
  });
}
...
function buildChannel(
  ...
  specs: DemoFrameSpec[],
  startIndex: number,
): { channel: BlinkChannel; frames: BlinkFrame[] } {
  ...
  const frames = specs.map((spec, offset): BlinkFrame => {
    ...
    return {
      ...
      target: "NGC 6822",
      night: spec.night,
      path: `${DEMO_ROOT}/${filter}/${stem}.fits`,
      name: `${stem}.fits`,
      sourceSha256: digest(stem),
      observedAt: `${spec.night}T${spec.time.replaceAll("-", ":")}`,
      airmass: 1.3,
      reference: spec.reference === true,
      defaultDecision,
      flags: spec.flags,
      notes: spec.notes ?? [],
      ...
      metrics: {
        sky: spec.sky,
        skyRatio: Number(skyRatio.toFixed(2)),
        starCount: spec.stars,
        sourceRatio: Number((spec.stars / sourcesBest).toFixed(2)),
        extinctionMag: spec.extinction,
        transparency: spec.transparency,
        fwhmNative: spec.fwhm,
        fwhmRatio: Number((spec.fwhm / fwhmBest).toFixed(2)),
        ellipticity: spec.ellipticity,
        eccentricity: Number(Math.sqrt(1 - (1 - spec.ellipticity) ** 2).toFixed(2)),
        registrationRms: registered ? 0.21 : null,
        matchedStars: registered ? Math.round(spec.stars * 0.33) : null,
        overlap: registered ? 0.99 : null,
        backgroundShape: spec.shape,
        gradientRatio: null,
      },
      ...
      transformToReference: registered
        ? [
            [1, 0, Number((offset * 0.7 - 2).toFixed(2))],
            [0, 1, Number((1.1 - offset * 0.4).toFixed(2))],
          ]
        : null,
  ...
  const nights = [...new Set(specs.map((spec) => spec.night))].map((night): BlinkNightSummary => {
    ...
    return {
      ...
      medianSky: median(own.map((frame) => frame.metrics.sky ?? 0)),
      skyRatio: Number(median(own.map((frame) => frame.metrics.skyRatio ?? 0)).toFixed(2)),
      medianSourceRatio: Number(median(own.map((frame) => frame.metrics.sourceRatio ?? 0)).toFixed(2)),
      medianExtinction: Number(median(own.map((frame) => frame.metrics.extinctionMag ?? 0)).toFixed(2)),
      exclude,
      attention: own.filter((frame) => frame.defaultDecision === "KEEP" && frame.flags.length > 0).length,
      defaultDropNight: exclude === own.length,
    };
  });
  ...
  const channel: BlinkChannel = {
    ...
    reference: reference
      ? { index: reference.index, sourceSha256: reference.sourceSha256, rule: "psf-signal-weight-proxy-v1" }
      : null,
    statistics: {
      skyClean,
      cleanCount: frames.filter((frame) => frame.flags.length === 0).length,
      sourcesBest,
      fwhmBest,
    },
    stretch: {
      black: skyClean - 2.5 * 43.3,
      white: skyClean + 10 * 43.3,
      softness: 4,
...
const MOONLIT = (sky: number, stars: number): BlinkFlag[] => [
  flag(
    "BLINK_SKY_BRIGHT",
    "EXCLUDE",
    Number((sky / 1090).toFixed(2)),
    1.6,
    `Sky ${(sky / 1090).toFixed(2)}× the channel's clean-sky level and ${Math.round((100 * stars) / 5680)} % of its stars: moonlit or hazy night`,
    true,
  ),
  flag(
    "BLINK_SOURCES_LOW",
    "ATTENTION",
    Number((stars / 5680).toFixed(2)),
    0.6,
    `${Math.round((100 * stars) / 5680)} % of the channel's best star count`,
    true,
  ),
];
...
export function demoBlinkManifest(): BlinkMeasureResponse {
    ...
    [
      {
        ...
        rank: 7,
        z: -0.9,
        flags: [flag("BLINK_SOURCES_LOW", "ATTENTION", 0.57, 0.6, "57 % of the channel's best star count")],
      },
      {
        time: "22-41-36",
        night: "2026-08-17",
      ...
      },
      {
        ...
        flags: [
          flag(
            ...
            1.02,
            0.5,
            "Background shape differs from the reference by 1.02 σ (P95−P5 of the outer cells)",
          ),
        ],
      },
      {
      ...
      },
      {
        ...
        registered: false,
        flags: [
          flag("BLINK_UNREGISTRABLE", "EXCLUDE", null, null, "No registration transform was found"),
          flag("BLINK_FEW_STARS", "EXCLUDE", 6, 20, "6 stars detected"),
          flag("BLINK_CLOUD_THICK", "EXCLUDE", null, null, "Multi-family cloud evidence (gate HARD_FAIL)"),
        ],
      ...
      },
      {
        ...
        z: -1.4,
        flags: [
          flag("BLINK_EXTINCTION", "ATTENTION", 0.67, 0.5, "0.67 mag of extra extinction: light cloud"),
          flag(
            "BLINK_BACKGROUND_SHAPE",
            "ATTENTION",
            0.58,
            0.5,
            "Background shape differs from the reference by 0.58 σ",
          ),
        ],
    ...
    [
      {
        ...
        z: -1.2,
        flags: [
          flag("BLINK_EXTINCTION", "ATTENTION", 0.54, 0.5, "0.54 mag of extra extinction: light cloud"),
          flag("BLINK_SOURCES_LOW", "ATTENTION", 0.57, 0.6, "57 % of the channel's best star count"),
        ],
      },
      {
        time: "21-37-32",
      ...
      },
      {
        ...
        registered: false,
        flags: [
          flag("BLINK_SOURCES_LOW", "EXCLUDE", 0.15, 0.45, "15 % of the channel's best star count"),
          flag("BLINK_EXTINCTION", "EXCLUDE", 1.9, 1, "1.90 mag of extra extinction: thick cloud"),
          flag("BLINK_UNREGISTRABLE", "EXCLUDE", null, null, "No registration transform was found"),
        ],
  ...
  return {
    schemaVersion: 1,
    kind: "blink-manifest-v1",
    sessionId: "demo-blink-session",
    sessionDirectory: `${DEMO_ROOT}/session`,
    createdAt: "2026-09-22T00:00:00Z",
    engineVersion: "demo",
    gatePolicyDigest: `sha256:${"7".repeat(64)}`,
    flagsPolicyDigest: `sha256:${"b".repeat(64)}`,
    flagsPolicy: {
      version: "blink-flags-v1",
      skyBrightAttention: 1.6,
      sourcesLowAttention: 0.6,
      sourcesLowExclude: 0.45,
      extinctionAttention: 0.5,
      extinctionExclude: 1,
    },
    inventorySha256: `sha256:${"1".repeat(64)}`,
    timings: { measurementSeconds: 0, analysisSeconds: 0, gateSeconds: 0, flagsSeconds: 0, previewSeconds: 0 },
    counts: {
      ...
      clean: frames.filter((frame) => frame.flags.length === 0).length,
    },
    channels: [luminance.channel, red.channel],
    frames,
