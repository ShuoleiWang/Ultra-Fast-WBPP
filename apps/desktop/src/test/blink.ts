...
export async function paintStage() {
  for (const image of screen.getAllByTestId("blink-stage-decoder")) {
    Object.defineProperties(image, {
      ...
      naturalWidth: { configurable: true, value: 782 },
      naturalHeight: { configurable: true, value: 522 },
    });
...
export async function reviewChannel(confirmName: RegExp) {
  ...
  await waitFor(() => {
    if (confirm.hasAttribute("disabled")) throw new Error("channel is not reviewed");
  });
...
export function inventoryBlinkManifest(assets: InspectedAsset[]): BlinkMeasureResponse {
  ...
  const frames = lights.map((asset, index) => ({
    ...
    target: asset.target,
    index,
    channelId: `${asset.target}-${asset.filter}`,
    sourceSha256: `sha256:${(index + 1).toString(16).padStart(64, "0")}`,
    reference: false,
    flags: [],
