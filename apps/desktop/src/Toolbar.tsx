    ...
    workflow.runStatus === "CANCELLED"
      ...
      : workflow.runStatus === "FAILED"
        ...
        : workflow.runStatus === "COMPLETED"
          ? t("resultTitle")
          : runningStage
            ? stageLabel(runningStage.stageId, t)
            : t("runningTitle");
  const groups = workflow.matrix.length;
  return (
    <header className="topbar" data-tauri-drag-region="deep">
      <div className="tb-left">
        <div className="brand" aria-label="Ultra-Fast WBPP">
          <span className="brand-mark">
            <BrandMark size={22} />
          </span>
          <span>
            Ultra-Fast <em>WBPP</em>
          </span>
        </div>
        ...
        <button
          ...
          onClick={() => void workflow.pickFiles(workflow.selectedRole)}
        >
          <FilesIcon />
          <span className="label">{t("chooseFiles")}</span>
        </button>
        <button
          type="button"
          className="btn"
          ...
          disabled={importDisabled}
          onClick={() => void workflow.pickDirectories(workflow.selectedRole)}
        >
          <FolderIcon />
          <span className="label">{t("chooseFolder")}</span>
        </button>
        {workflow.importedTotal > 0 && (
          <button
            type="button"
            className="btn quiet danger"
            disabled={workflow.inputBusy || workflow.runNavigationLocked}
            title={t("clearTitle")}
            onClick={workflow.clearSources}
          >
            {t("clear")}
          </button>
        )}
      ...
      <div className="tb-center">
        {showActivity ? (
          <div
            className={`activity ${workflow.runStatus === "COMPLETED" ? "done" : ""}`}
            role="progressbar"
            aria-label={`${workflow.overallProgress}%`}
            aria-valuenow={workflow.overallProgress}
            aria-valuemin={0}
            aria-valuemax={100}
          >
            <div className="bar">
              <span style={{ width: `${workflow.overallProgress}%` }} />
            </div>
            <span className="txt tnum">
              {workflow.overallProgress}% · {activityText}
            </span>
          </div>
        ) : workflow.blinkBusy ? (
          ...
          <>
            ...
            <div className="doc-sub">
              {workflow.importedTotal > 0
                ? t("statusFrames", { count: workflow.importedTotal, groups })
                : t("newProject")}
            </div>
          </>
        )}
      ...
      <div className="tb-right">
        <label className="language-picker" data-tauri-drag-region="false">
