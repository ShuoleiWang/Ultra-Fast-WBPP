...
describe("human Blink screening", () => {
  it("requires review, opens paused in time order, and treats flags as advice", async () => {
    ...
    await screen.findByRole("heading", { name: /Blink and select/ });
    expect(native.blinkMeasure).toHaveBeenCalledWith({ paths: lightPaths(), masterFlats: [], masterDarks: [] } satisfies BlinkMeasureRequest);
  ...
  it("steps, decides with K/D, jumps to flags/reference, and undoes whole-night/range changes", async () => {
    render(<App />); await reachBlink();
    const first = currentTile(); await paintStage(); key("d");
    expect(first).toHaveAttribute("aria-pressed", "false");
    expect(currentTile()).not.toBe(first);
    await paintStage(); key("k");
    expect(currentTile()).toBe(tiles()[2]);
    key("Home"); key("ArrowLeft"); expect(currentTile()).toBe(tiles().at(-1));
    key("r"); expect(currentTile()).toHaveAttribute("aria-label", L("2026-08-17_22-57-01"));
    key("f"); expect(currentTile()).not.toHaveAttribute("aria-label", L("2026-08-17_22-57-01"));
    key("z"); expect(first).toHaveAttribute("aria-pressed", "true");
    fireEvent.click(screen.getAllByRole("button", { name: "Drop night" })[0]);
    expect(tiles().slice(0, 7).every((tile) => tile.getAttribute("aria-pressed") === "false")).toBe(true);
    key("z", { metaKey: true });
    expect(tiles().every((tile) => tile.getAttribute("aria-pressed") === "true")).toBe(true);
    fireEvent.click(tiles()[0]); fireEvent.click(tiles()[3], { shiftKey: true });
    expect(tiles().slice(0, 4).every((tile) => tile.getAttribute("aria-pressed") === "false")).toBe(true);
    key("z"); expect(tiles().every((tile) => tile.getAttribute("aria-pressed") === "true")).toBe(true);
  ...
  it("waits for a painted image before advancing playback and stops at the channel end", async () => {
    render(<App />); await reachBlink();
    vi.useFakeTimers(); key("p");
    await act(async () => { vi.advanceTimersByTime(2_000); });
    expect(currentTile()).toBe(tiles()[0]);
    const image = screen.getByTestId("blink-stage-decoder");
    Object.defineProperties(image, { complete: { value: true }, naturalWidth: { value: 782 }, naturalHeight: { value: 522 } });
    ...
    await act(async () => { vi.advanceTimersByTime(2_000); });
    expect(currentTile()).toBe(tiles()[1]);
    key("End"); expect(screen.getByRole("button", { name: "Play" })).toBeInTheDocument();
    vi.useRealTimers(); await paintStage();
    // Restart from the beginning, without treating prefetched thumbnails as viewed.
    key("p"); expect(currentTile()).toBe(tiles()[0]);
    key("Escape"); expect(chip("L")).toHaveTextContent("2/14 viewed");
  });

  it("does not count hidden previews and marks the frame when the view becomes visible", async () => {
    const hidden = vi.spyOn(document, "hidden", "get").mockReturnValue(true);
    render(<App />); await reachBlink(); await paintStage();
    expect(chip("L")).toHaveTextContent("0/14 viewed");
  ...
  it("shows failed previews, retries them, and never counts a decode error as viewed", async () => {
    ...
    await screen.findByTestId("blink-stage-decoder"); await paintStage();
    expect(chip("L")).toHaveTextContent("1/14 viewed");
    key("ArrowRight"); fireEvent.error(screen.getByTestId("blink-stage-decoder"));
    await screen.findByText("This preview could not be displayed");
    key("d"); expect(chip("L")).toHaveTextContent("2/14 viewed");
  ...
  it("shares zoom/pan in reference comparison and loads detailed images on demand", async () => {
    const context = mockPreviewCanvas(); render(<App />); await reachBlink(); await paintStage();
    key("+"); key("+");
    await waitFor(() => expect(native.loadBlinkPreview).toHaveBeenCalledWith(manifest().sessionDirectory, manifest().frames[0].previews.zoom));
    await paintStage(); const scale = context.scale.mock.lastCall;
    key("ArrowRight"); await paintStage(); expect(context.scale.mock.lastCall).toEqual(scale);
    fireEvent.click(screen.getByRole("button", { name: "Compare with reference" })); await paintStage();
    expect(document.querySelectorAll(".blink-pane")).toHaveLength(2);
    expect(stage().getAllByRole("img")[0]).toHaveAttribute("aria-label", expect.stringContaining(L("2026-08-17_22-57-01")));
    expect(context.scale.mock.calls.at(-1)).toEqual(context.scale.mock.calls.at(-2));
    key("2"); expect(chip("R")).toHaveAttribute("aria-pressed", "true");
    expect(screen.getByRole("button", { name: "Fit" })).toHaveAttribute("aria-pressed", "true");
    key("1"); expect(chip("L")).toHaveAttribute("aria-pressed", "true");
  });

  it("blocks a reviewed channel with fewer than two kept Lights", async () => {
    render(<App />); await reachBlink();
    await userEvent.click(screen.getByRole("button", { name: "Choose output folder" }));
    await reviewChannel(/Confirm L/); await reviewChannel(/Confirm R/);
    for (const tile of tiles().slice(1)) { fireEvent.click(tile); key(" "); }
    fireEvent.click(screen.getByRole("button", { name: /Confirm R/ }));
  ...
  it("sends the exact manual decisions and completed review identity to the controller", async () => {
    render(<App />); await reachBlink(); await paintStage(); key("d");
    await reviewChannel(/Confirm L/); await reviewChannel(/Confirm R/);
    await userEvent.click(screen.getByRole("button", { name: "Choose output folder" }));
    ...
    expect(request.selection).toMatchObject({ policy: "explicit-v1", undecided: "ERROR", origin: { blinkManifestSha256: manifest().manifestSha256 } });
    ...
    expect(request.selection!.decisions.some((item) => item.defaultDecision === "DROP" && item.decision === "KEEP")).toBe(true);
    expect(new Set(request.blinkReview!.reviewedSourceSha256s)).toEqual(new Set(manifest().frames.map((frame) => frame.sourceSha256)));
    expect(request.blinkReview!.confirmedChannelIds).toEqual(manifest().channels.map((channel) => channel.channelId));
    await act(async () => native.handlers?.onComplete({ jobId: "run-1", outputDirectory: "/results/deep sky/run", artifacts: [solvedArtifact], gate: { decision: "ready", checks: readyChecks }, screening: { admitted: 21, excluded: 1, counts: { PASS: 20, REVIEW: 0, HARD_FAIL: 2 }, frames: [{ name: L("2026-08-17_22-36-34"), target: "NGC 6822", disposition: "PASS", admitted: false, summary: "", evidence: [], starCount: 1200, reason: "USER_DROP", flags: [] }] } }));
  ...
  it("retains decisions on calibration additions, resets on Light reimport, and forwards all masters", async () => {
    render(<App />); await reachBlink(); await paintStage(); key(" ");
    await userEvent.click(screen.getByRole("button", { name: "Back" }));
    const extra = [asset("/data/calibration/master flat L.xisf", "L", "MASTER_FLAT"), { ...asset("/data/calibration/master dark.fit", "L", "MASTER_DARK"), exposureSeconds: 300 }, asset("/data/calibration/master bias.fit", "L", "MASTER_BIAS")];
    native.inspectPaths.mockResolvedValueOnce({ projectName: "NGC 6822", totalFiles: extra.length, assets: extra, sources: extra.map((a) => ({ role: a.role, paths: [a.path], fileCount: 1, confidence: 1, needsConfirmation: false })) });
    await userEvent.click(screen.getByRole("button", { name: /Choose folder$/ }));
    await userEvent.click(screen.getAllByRole("button", { name: "Open blink selection" })[0]);
    expect(native.blinkMeasure).toHaveBeenCalledTimes(1); expect(tiles()[0]).toHaveAttribute("aria-pressed", "false");
    await userEvent.click(screen.getByRole("button", { name: "Back" })); await importLights();
    await userEvent.click(screen.getByRole("button", { name: "Blink & select (22 Lights)" }));
    await screen.findByRole("heading", { name: /Blink and select/ });
    expect(chip("L")).toHaveTextContent("0/14 viewed"); expect(tiles()[0]).toHaveAttribute("aria-pressed", "true");
...
describe("browser demo", () => {
  it("opens the bundled session without native calls", async () => {
    native.runtime = false; window.history.replaceState({}, "", "/?demo=blink"); render(<App />);
    await screen.findByRole("heading", { name: /Blink and select/ });
    expect(native.blinkMeasure).not.toHaveBeenCalled(); expect(tiles()).toHaveLength(14);
    await paintStage(); expect(chip("L")).toHaveTextContent("1/14 viewed");
  });
});
