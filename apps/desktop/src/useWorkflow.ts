...
import {
  ...
  inventoryKeyOf,
  known,
  masterOverrideRequests,
  monoCfa,
  panelMatrix,
  startBlockers,
  rememberOutputParent,
  runLabel,
  safeSourceId,
  solverScienceReady,
  storedOutputParent,
  unknownCfa,
  type BlinkChannelSummary,
  type BlinkSession,
  type SolverBackendId,
} from "./workflow/model";
export { runLabel, solverScienceReady } from "./workflow/model";
export type { BlinkChannelSummary, BlinkSession, SolverBackendId, StartBlocker } from "./workflow/model";
...
export function useWorkflow(t: Translator) {
  ...
  const [outputParent, setOutputParentState] = useState<string | undefined>(() =>
    nativeRuntime ? storedOutputParent() : undefined,
  );
  const setOutputParent = (value: string | undefined) => {
    setOutputParentState(value);
    if (nativeRuntime) rememberOutputParent(value);
  };
  ...
  const [catalogStatus, setCatalogStatus] = useState<
    "IDLE" | "DOWNLOADING" | "VERIFYING" | "READY" | "CANCELLED" | "FAILED"
  >("IDLE");
  ...
  const startRunClock = () => {
    ...
    runClockRef.current = window.setInterval(
      () => setRunElapsedSeconds(Math.floor((performance.now() - startedAt) / 1000)),
      1000,
    );
  };
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(
      () => setQualityElapsedSeconds(Math.floor((performance.now() - started) / 1000)),
      1000,
    );
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(
      () => setBlinkElapsedSeconds(Math.floor((performance.now() - started) / 1000)),
      1000,
    );
    return () => window.clearInterval(timer);
  }, [blinkBusy]);
  ...
  const clearBlinkSession = useCallback(() => {
    setBlinkSession(undefined);
    blinkSessionRef.current = undefined;
  ...
  const refreshSolverSetup = useCallback(async () => {
    ...
    try {
      const [listing, catalogState, solverState] = await Promise.allSettled([
        desktopBridge.catalogList(),
        desktopBridge.catalogDoctor(),
        desktopBridge.solverDoctor(),
      ]);
      ...
      const failures = [listing, catalogState, solverState].flatMap((result) =>
        result.status === "rejected" ? [String(result.reason)] : [],
      );
      if (failures.length) throw new Error(failures.join("\n"));
    } finally {
      setSolverSetupBusy(false);
    }
  }, [nativeRuntime]);

  const importPaths = useCallback(
    async (paths: string[], roleHint?: FrameRole) => {
      if (
        !paths.length ||
        !nativeRuntime ||
        stepRef.current !== "import" ||
        inventoryInFlightRef.current ||
        qualityInFlightRef.current ||
        blinkInFlightRef.current ||
        runLaunchInFlightRef.current
      )
        return;
      ...
      try {
        ...
        const priorLights = new Set(
          assetsRef.current.filter((asset) => asset.role === "LIGHT").map((asset) => asset.path),
        );
        const touchesLights = inspected.sources.some(
          (source) => source.role === "LIGHT" || source.paths.some((path) => priorLights.has(path)),
        );
        ...
        if (touchesLights) {
          ...
          setGate({ pass: 0, review: 0, hardFail: 0 });
          clearBlinkSession();
        }
  ...
  useEffect(() => {
    ...
    setMasterOverrides((current) =>
      masters.map((master) =>
        buildMasterOverride(
          master,
          current.find((item) => item.sourcePath === master.path && item.sourceSha256 === master.sourceSha256),
        ),
      ),
    );
  }, [assets]);
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      desktopBridge
        .inspectCalibration(JSON.parse(calibrationRequestKey))
        .then((report) => {
          if (current) setCalibrationResult({ key: calibrationCheckKey, report });
        })
        .catch((error) => {
          if (current) setCalibrationResult({ key: calibrationCheckKey, error: String(error) });
        });
    }, 150);
    return () => {
      current = false;
  ...
  const calibrationInspection =
    !inventoryBusy && calibrationResult?.key === calibrationCheckKey ? calibrationResult.report : undefined;
  ...
  useEffect(() => {
    ...
    let disposed = false;
    if (nativeRuntime)
      listenForDesktopDrops((paths) => void importPaths(paths, selectedRole)).then((dispose) => {
        if (disposed) dispose();
  ...
  const handleProgress = useCallback((event: PipelineProgressEvent) => {
    if (!jobIdRef.current) {
      if (runLaunchInFlightRef.current) pendingProgressRef.current.push(event);
      return;
    }
    ...
    setStages((current) => {
      ...
      return active.map((stage, index) => {
        ...
        return {
          ...stage,
          percent: Math.max(stage.percent, fraction * 100),
          status: event.state === "failed" ? "FAILED" : event.state === "succeeded" ? "DONE" : "RUNNING",
        };
      });
    });
    const legacyIndex = STAGE_DEFINITIONS.findIndex((stage) => stage.stageId === event.stageId);
    const overall =
      event.overallFraction !== undefined && Number.isFinite(event.overallFraction)
        ? event.overallFraction
        : legacyIndex >= 0
          ? (legacyIndex + fraction) / STAGE_DEFINITIONS.length
  ...
  const failStages = (current: StageProgress[]) =>
    current.map((stage) => (stage.status === "RUNNING" ? { ...stage, status: "FAILED" as const } : stage));

  const handleComplete = useCallback(
    (event: PipelineCompleteEvent, receivedAt = performance.now()) => {
      if (!jobIdRef.current) {
        if (runLaunchInFlightRef.current) pendingTerminalRef.current ??= { kind: "complete", event, receivedAt };
        return;
      }
      if (event.jobId !== jobIdRef.current || event.jobId === terminalJobIdRef.current) return;
      if (cancellingRef.current) {
        pendingTerminalRef.current ??= { kind: "complete", event, receivedAt };
        return;
      }
      terminalJobIdRef.current = event.jobId;
      stopRunClock(receivedAt);
      const required = event.gate.checks.filter((check) => check.required);
      const solvedProducts = event.artifacts.filter(
        (artifact) => artifact.kind === "SOLVED_MONO_FITS" || artifact.kind === "MASTER",
      );
      const gateReady =
        event.gate.decision === "ready" && required.length > 0 && required.every((check) => check.passed);
      const solvedReady =
        solvedProducts.length > 0 && solvedProducts.every((artifact) => Boolean(artifact.receipt?.astrometry));
  ...
  const handleError = useCallback(
    (event: PipelineErrorEvent, receivedAt = performance.now()) => {
      if (!jobIdRef.current) {
        if (runLaunchInFlightRef.current) pendingTerminalRef.current ??= { kind: "error", event, receivedAt };
        return;
      }
      if (event.jobId !== jobIdRef.current || event.jobId === terminalJobIdRef.current) return;
      if (cancellingRef.current) {
        pendingTerminalRef.current ??= { kind: "error", event, receivedAt };
        return;
      }
      terminalJobIdRef.current = event.jobId;
      stopRunClock(receivedAt);
  ...
  useEffect(() => {
    ...
    let disposed = false;
    listenForPipelineEvents({ onProgress: handleProgress, onComplete: handleComplete, onError: handleError }).then(
      (value) => {
        if (disposed) value();
        else dispose = value;
      },
  ...
  const finishCatalogInstall = useCallback(
    async (event: CatalogCompleteEvent) => {
  ...
  const confirmRole = (role: FrameRole) =>
    setSources((current) => current.map((source) => (source.role === role ? { ...source, confirmed: true } : source)));
  const pickFiles = async (role?: FrameRole) => {
    try {
      await importPaths(await desktopBridge.pickInputFiles(role), role);
    } catch (error) {
      setErrorMessage(String(error));
    }
  };
  const pickDirectories = async (role?: FrameRole) => {
    try {
      await importPaths(await desktopBridge.pickInputDirectories(role), role);
    } catch (error) {
      setErrorMessage(String(error));
    }
  };
  const chooseOutputParent = async () => {
    try {
      const selected = await desktopBridge.pickOutputParent();
      if (selected) setOutputParent(selected);
    } catch (error) {
      setErrorMessage(String(error));
    }
  };
  const useOutputParentPath = (path: string) => {
    if (
      !nativeRuntime ||
      !["import", "inspect", "blink"].includes(stepRef.current) ||
      runLaunchInFlightRef.current ||
      runStatus === "RUNNING" ||
  ...
  const updateMasterOverride = (digest: string, patch: Partial<MasterMetadataOverride>) =>
    setMasterOverrides((current) =>
      current.map((item) =>
        item.sourceSha256 === digest ? { ...item, ...patch, needsMetadataOverride: true, confirmed: false } : item,
      ),
    );
  const resetMasterOverride = (digest: string) =>
    setMasterOverrides((current) =>
      current.map((item) => {
  ...
  const confirmMasterOverride = (digest: string) =>
    setMasterOverrides((current) =>
      current.map((item) => {
        ...
        const binningReady =
          item.binning.every((value) => value === null) ||
          item.binning.every((value) => value !== null && Number.isInteger(value) && value > 0);
        const numericDomainReady =
          item.numericDomain === null ||
          (item.normalizedUnitScale !== null &&
            Number.isFinite(item.normalizedUnitScale) &&
            item.normalizedUnitScale > 0);
  ...
  const loadDemo = () => {
    ...
    setProjectName("Clearly labelled browser demo");
    setSources(
      emptySources().map((source) => ({
        ...source,
        paths: DEMO_COUNTS[source.role] ? [`/explicit-browser-demo/${source.role.toLowerCase()}`] : [],
        fileCount: DEMO_COUNTS[source.role],
        detected: DEMO_COUNTS[source.role] > 0,
        confirmed: source.role !== "LIGHT",
      })),
    );
    setAssets(
      Array.from({ length: 4 }, (_, panel) =>
        ["R", "G", "B", "L"].flatMap((filter) =>
          Array.from({ length: 4 }, (_, index): InspectedAsset => ({
            path: `/explicit-browser-demo/p${panel + 1}/${filter}/${index}.fit`,
            role: "LIGHT",
            width: 6248,
            height: 4176,
            channels: 1,
    ...
    setGate({ pass: 347, review: 23, hardFail: 0 });
    setSelectedRole("LIGHT");
    setErrorMessage(undefined);
  };
  ...
  const runInspection = async (force = false) => {
    ...
    try {
      ...
      setGate({
        pass: inspection.counts.PASS,
        review: inspection.counts.REVIEW,
        hardFail: inspection.counts.HARD_FAIL,
      });
      setStep("inspect");
    } catch (error) {
      setErrorMessage(t("qcPreflightFailed", { error: String(error) }));
    } finally {
  ...
  const runBlink = async (force = false) => {
    if (demoMode && !nativeRuntime) {
      if (!blinkSession) installBlinkSession(demoBlinkManifest(), [], true);
      setStep("blink");
      return;
    }
    const lightPaths = sources.find((source) => source.role === "LIGHT")?.paths ?? [];
    ...
    setErrorMessage(undefined);
    try {
      const masterFlats = assets
        .filter((asset) => asset.role === "MASTER_FLAT" && known(asset.filter))
        .map((asset) => ({ filter: asset.filter, path: asset.path }));
      const masterDarks = assets
        .filter((asset) => asset.role === "MASTER_DARK")
        .map((asset) => ({
          path: asset.path,
          ...(asset.exposureSeconds ? { exposureSeconds: asset.exposureSeconds } : {}),
        }));
      const biases = assets.filter((asset) => asset.role === "MASTER_BIAS");
      const manifest = await desktopBridge.blinkMeasure({
        paths: lightPaths,
        masterFlats,
        masterDarks,
        ...(biases.length === 1 ? { masterBias: biases[0].path } : {}),
      });
      installBlinkSession(manifest, lightPaths, false);
      setStep("blink");
    } catch (error) {
      setErrorMessage(t("blinkFailed", { error: String(error) }));
    } finally {
  ...
  const invalidateReviewedChannels = (changed: string[]) => {
    const ids = new Set(
      blinkSessionRef.current?.manifest.frames.filter((f) => changed.includes(f.sourceSha256)).map((f) => f.channelId),
    );
    const next = Object.fromEntries(
      Object.entries(confirmedChannelsRef.current).filter(([id]) => !ids.has(id)),
    ) as Record<string, true>;
    confirmedChannelsRef.current = next;
    setConfirmedChannels(next);
  };
  const markFrameViewed = useCallback((digest: string, layer: "field" | "background" = "field") => {
    ...
    layers[layer] = true;
    paintedLayersRef.current[digest] = layers;
    ...
    viewedFramesRef.current = next;
    setViewedFrames(next);
  }, []);
  const reportPreviewFailure = useCallback((digest: string, error: string | undefined, layer = "field") => {
    ...
    if (error) next[digest] = error;
    else delete next[digest];
    previewFailuresRef.current = next;
    setPreviewFailures(next);
  }, []);
  const confirmBlinkChannel = (channelId: string) => {
    ...
    if (
      ...
      frames.some(
        (f) =>
          !viewedFramesRef.current[f.sourceSha256] ||
          (previewFailuresRef.current[f.sourceSha256] && decisionsRef.current[f.sourceSha256] !== "DROP"),
      )
    )
      return;
  ...
  const commitDecisions = (
    update: (current: Record<string, BlinkDecision>) => Record<string, BlinkDecision> | undefined,
  ) => {
    ...
    decisionHistoryRef.current = history;
    setDecisionHistory(history);
  };
  const setDecision = (sourceSha256: string, decision: BlinkDecision) => {
    commitDecisions((current) =>
      !(sourceSha256 in current) || current[sourceSha256] === decision
        ? undefined
        : { ...current, [sourceSha256]: decision },
    );
    ...
    if (decision === "DROP" && previewFailuresRef.current[sourceSha256]) {
      ...
      markFrameViewed(sourceSha256, "field");
      markFrameViewed(sourceSha256, "background");
    }
  };
  const toggleDecision = (sourceSha256: string) => {
    if (sourceSha256 in decisionsRef.current)
      setDecision(sourceSha256, decisionsRef.current[sourceSha256] === "KEEP" ? "DROP" : "KEEP");
  };
  // A filmstrip range (shift-click): one undo step for the whole range.
  const setDecisionsBulk = (sourceSha256s: string[], decision: BlinkDecision) =>
    commitDecisions((current) => {
      const changed = sourceSha256s.filter((digest) => digest in current && current[digest] !== decision);
      return changed.length
        ? { ...current, ...Object.fromEntries(changed.map((digest) => [digest, decision])) }
        : undefined;
    });
  const setNightDecision = (channelId: string, night: string, decision: BlinkDecision) =>
    commitDecisions((current) => {
      const frames = (blinkSessionRef.current?.manifest.frames ?? []).filter(
        (frame) => frame.channelId === channelId && frame.night === night && current[frame.sourceSha256] !== decision,
      );
      return frames.length
        ? { ...current, ...Object.fromEntries(frames.map((frame) => [frame.sourceSha256, decision])) }
        : undefined;
    });
  const undo = () => {
    ...
    decisionHistoryRef.current = history.slice(0, -1);
    setDecisionHistory(decisionHistoryRef.current);
    invalidateReviewedChannels(
      Object.keys(previous).filter((digest) => previous[digest] !== decisionsRef.current[digest]),
    );
    decisionsRef.current = previous;
    setDecisions(previous);
  };
  const selectBlinkChannel = (channelId: string) => {
    if (blinkSessionRef.current?.manifest.channels.some((channel) => channel.channelId === channelId))
      setBlinkChannel(channelId);
  };
  ...
  const startDemo = () => {
    startRunClock();
    terminalJobIdRef.current = undefined;
    cancellingRef.current = false;
    setJobId("explicit-browser-demo");
    jobIdRef.current = "explicit-browser-demo";
    setExecutionMode("demo");
    ...
    timerRef.current = window.setInterval(
      () =>
        setOverallProgress((current) => {
          ...
          const segment = 100 / STAGE_DEFINITIONS.length;
          setStages(
            initialStages().map((stage, index) => {
              const local = Math.max(0, Math.min(100, ((next - segment * index) / segment) * 100));
              return { ...stage, percent: local, status: local >= 100 ? "DONE" : local > 0 ? "RUNNING" : "WAITING" };
            }),
          );
          if (next === 100) {
            window.clearInterval(timerRef.current);
  ...
  const startRun = async () => {
    ...
    runLaunchInFlightRef.current = true;
    setRunLaunchBusy(true);
    jobIdRef.current = undefined;
    pendingProgressRef.current = [];
    pendingTerminalRef.current = undefined;
    terminalJobIdRef.current = undefined;
    progressContextRef.current = undefined;
    cancellingRef.current = false;
    setJobId(undefined);
    ...
    try {
      ...
      const receipt = await desktopBridge.startRun({
        sources: sources
          .filter((source) => source.paths.length)
          .flatMap((source) =>
            source.paths.map((path) => ({
              sourceId: safeSourceId(source.role, sourceIndex++),
              role: source.role,
              paths: [path],
              recursive: false,
            })),
      ...
      jobIdRef.current = receipt.jobId;
      setJobId(receipt.jobId);
      setOutputDirectory(receipt.outputDirectory);
      ...
      runLaunchInFlightRef.current = false;
      setRunLaunchBusy(false);
    }
  };
  ...
  const cancelRun = async () => {
    ...
    try {
      ...
      pendingTerminalRef.current = undefined;
      terminalJobIdRef.current = jobId;
      stopRunClock();
  ...
  const recommendedCatalog = useMemo<CatalogInfo | undefined>(
    () => catalogList?.catalogs.find((item) => item.catalogId === "astrometry-net-4107-4112"),
    [catalogList],
  );
  const startCatalogInstall = async () => {
    if (!recommendedCatalog || !catalogTermsAccepted || catalogStatus === "DOWNLOADING") return;
    setCatalogError(undefined);
    setCatalogProgress(undefined);
    setCatalogStatus("DOWNLOADING");
    catalogCancellingRef.current = false;
    catalogJobIdRef.current = undefined;
    pendingCatalogCompleteRef.current = undefined;
    try {
      const receipt = await desktopBridge.startCatalogInstall({
        catalogId: recommendedCatalog.catalogId,
        acceptedTermsId: recommendedCatalog.providerTerms.acceptanceId,
      });
      catalogJobIdRef.current = receipt.jobId;
      setCatalogJobId(receipt.jobId);
      const pending = pendingCatalogCompleteRef.current;
      pendingCatalogCompleteRef.current = undefined;
      if (pending) await finishCatalogInstall(pending);
    } catch (error) {
      setCatalogStatus("FAILED");
  ...
  const cancelCatalogInstall = async () => {
    ...
    try {
      await desktopBridge.cancelCatalogInstall(catalogJobId);
      setCatalogStatus("CANCELLED");
      setCatalogJobId(undefined);
      catalogJobIdRef.current = undefined;
    } catch (error) {
      catalogCancellingRef.current = false;
  ...
  const openCatalogTerms = async (url: string) => {
    try {
      await desktopBridge.openProviderTerms(url);
    } catch (error) {
      setCatalogError(String(error));
    }
  };
  ...
  const cfaBlockedAssets = assets.filter(
    (asset) => !unknownCfa(asset.cfaPattern) && !monoCfa(asset.cfaPattern) && !bayerCfa(asset.cfaPattern),
  );
  ...
  const qualityReady = Boolean(
    qualityInspection &&
    qualityInspection.frames.length === (sources.find((source) => source.role === "LIGHT")?.fileCount ?? 0),
  );
  ...
  const blinkReady = Boolean(
    blinkSession &&
    (blinkSession.demo ? demoMode && !nativeRuntime : blinkSessionCovers(blinkSession, lightSourcePaths)),
  );
  const blinkChannels = useMemo<BlinkChannelSummary[]>(
    () =>
      (blinkSession?.manifest.channels ?? []).map((channel) => {
  ...
  const blinkReviewComplete =
    blinkReady &&
    blinkChannels.length > 0 &&
    blinkChannels.every((c) => c.confirmed && c.viewed === c.total) &&
  ...
  const selectionForRun = (): SelectionFile | undefined => {
    ...
    const origin = CONTENT_DIGEST.test(digest)
      ? {
          sessionId: manifest.sessionId,
          blinkManifestSha256: digest,
          flagsPolicyDigest: manifest.flagsPolicyDigest,
    ...
    return {
      ...
      ...(origin ? { origin } : {}),
      undecided: "ERROR",
      decisions: manifest.frames.map((frame) => ({
        sourceSha256: frame.sourceSha256,
        decision: decisions[frame.sourceSha256] ?? "KEEP",
        defaultDecision: frame.defaultDecision,
        flags: frame.flags.map((item) => item.code),
      })),
    };
  };
  const blinkAdmittedPaths = useMemo(
    () =>
      blinkReady && nativeRuntime
        ? new Set(
            blinkSession!.manifest.frames
              .filter((frame) => decisions[frame.sourceSha256] === "KEEP")
              .map((frame) => frame.path),
          )
        : undefined,
    [blinkReady, blinkSession, decisions, nativeRuntime],
  );
  const admittedPaths = useMemo(
    () => blinkAdmittedPaths ?? new Set(lightSourcePaths),
    [blinkAdmittedPaths, lightSourcePaths],
  );
  ...
  const insufficientBlinkPanels = blinkReady
    ? blinkChannels.filter((channel) => channel.kept < MINIMUM_KEPT_PER_CHANNEL)
    : [];
  ...
  const insufficientPanels = blinkAdmittedPaths
    ? matrix.filter(
        (cell) =>
          cell.admittedCount < minimumAdmittedLights ||
          insufficientBlinkPanels.some((channel) => channel.target === cell.target && channel.filter === cell.filter),
      )
    : matrix.filter((cell) => cell.lightCount < minimumAdmittedLights);
  const blockers = startBlockers({
    engineAvailable: Boolean(capabilities?.available),
    engineUnavailableReason: capabilities?.unavailableReason ?? undefined,
    calibrationReady,
    allRequiredConfirmed,
    panelCount: matrix.length,
    insufficientPanels,
    solverSetupReady,
    outputChosen: Boolean(outputParent),
    masterOverridesReady,
    cfaBlocked: cfaBlockedAssets.length > 0,
  });
  const runNavigationLocked = runLaunchBusy || runStatus === "RUNNING" || runStatus === "CANCELLING";
  const canStart =
    demoMode && !nativeRuntime
      ? !runNavigationLocked
      : !runNavigationLocked && !inputBusy && nativeRuntime && blockers.length === 0 && blinkReviewComplete;
  ...
  return {
    ...
    importedTotal,
    nativeRuntime,
    browserDemoAvailable: !nativeRuntime,
    pickFiles,
    pickDirectories,
    chooseOutputParent,
    useOutputParentPath,
    ...
    confirmBlinkChannel,
    blinkReviewComplete,
    insufficientBlinkPanels,
    minimumKeptPerChannel: MINIMUM_KEPT_PER_CHANNEL,
    loadBlinkPreview,
    calibrationInspection,
    calibrationBusy,
    calibrationError,
    ...
    primarySolver,
    solverSetupReady,
    catalogTermsAccepted,
    setCatalogTermsAccepted,
    catalogProgress,
    catalogStatus,
    catalogError,
    startCatalogInstall,
