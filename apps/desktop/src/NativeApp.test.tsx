...
const lightAsset = (path: string, filter = "R"): InspectedAsset => ({
  ...
  offset: 50,
  binning: [1, 1],
  cfaPattern: native.cfa ? "RGGB" : native.unknownCfa ? "UNKNOWN" : "NONE",
  readoutMode: "Mode 1",
  sourceSha256: null,
});
...
const inventory = () => {
  const assets: InspectedAsset[] = [
    ...Array.from({ length: native.lightCount }, (_, index) =>
      lightAsset(`/数据/盾牌座/亮场 ${String(index + 1).padStart(2, "0")}.fit`),
    ),
  ...
  if (native.useMasterDark) {
    assets.splice(
      assets.findIndex((asset) => asset.role === "DARK"),
      1,
      {
        ...lightAsset("/数据/校准/旧 MasterDark.fit"),
        role: "MASTER_DARK",
        target: "UNKNOWN",
        camera: "UNKNOWN",
        gain: null,
...
const catalogListing = {
  ...
  catalogRoot: "/Users/example/测试/.ultra-fast-wbpp/catalogs/astrometry-net",
  catalogs: [
    {
      catalogId: "astrometry-net-4107-4112",
      provider: "Astrometry.net",
      version: "checked",
      totalSizeBytes: 349_692_480,
      artifactCount: 6,
      installedArtifactsBySize: 0,
      fullyInstalledBySize: false,
      allowedDownloadOrigins: ["https://data.astrometry.net"],
      artifacts: [],
      providerTerms: {
        acceptanceId: "astrometry-net-index-data-2026-09",
        url: "https://astrometry.net/doc/readme.html#getting-index-files",
        summary: "Provider terms must be reviewed.",
...
vi.mock("./bridge", () => ({
  ...
  desktopBridge: {
    getCapabilities: vi.fn(async () => {
      ...
      if (native.platform === "windows")
        return {
          ...
          gpuBackend: "GPU acceleration not used",
          optimizationTier: "WINDOWS_X64",
          available: true,
    ...
    pickInputDirectories: vi.fn(async () => ["/数据/盾牌座 会话"]),
    pickOutputParent: vi.fn(async () => "/结果/深空 输出"),
    startRun: (...args: unknown[]) => native.startRun(...args),
    cancelRun: (...args: unknown[]) => native.cancelRun(...args),
    catalogList: vi.fn(async () => {
      native.solverSetupCalls += 1;
      return catalogListing;
    }),
    catalogDoctor: vi.fn(async () => {
      native.solverSetupCalls += 1;
      if (native.catalogInspectionError) throw native.catalogInspectionError;
      return {
        schemaVersion: 1,
        ok: native.catalogReady,
        catalogRoot: catalogListing.catalogRoot,
        config: { present: native.catalogReady, valid: native.catalogReady },
        installedSetBindingReady: native.catalogReady,
        message: native.catalogReady ? "ready" : "missing",
      };
    }),
    solverDoctor: vi.fn(async () => {
      native.solverSetupCalls += 1;
      if (native.solverError) throw native.solverError;
      return {
        schemaVersion: 1,
        engineVersion: "0.1.0",
        backends: [
          {
            ...
            version: "0.97",
            available: native.solverReady,
            executionReady: native.solverReady,
            metadata: {
              probe: { path: "/opt/homebrew/bin/solve-field", version: "0.97", executionReady: native.solverReady },
            },
          },
          native.astapReady
            ? {
                ...
                available: true,
                executionReady: true,
                scienceReady: native.astapScienceReady,
                reason: native.astapScienceReady
                  ? null
                  : "STAR_DATABASE_MISSING: no D20 or larger star database beside astap_cli.exe",
                metadata: {
                  probe: {
    ...
    startCatalogInstall: (...args: unknown[]) => native.startCatalogInstall(...args),
    cancelCatalogInstall: (...args: unknown[]) => native.cancelCatalogInstall(...args),
    verifyCatalog: (...args: unknown[]) => native.verifyCatalog(...args),
    openProviderTerms: (...args: unknown[]) => native.openProviderTerms(...args),
    revealOutput: (...args: unknown[]) => native.revealOutput(...args),
  },
  listenForDesktopDrops: vi.fn(async (onPaths: (paths: string[]) => void) => {
    native.dropListenerCalls += 1;
    native.dropHandler = onPaths;
    return () => undefined;
  }),
  listenForPipelineEvents: vi.fn(async (handlers: PipelineEventHandlers) => {
    native.pipelineListenerCalls += 1;
    native.handlers = handlers;
    return () => undefined;
  }),
  listenForCatalogEvents: vi.fn(async (handlers: CatalogEventHandlers) => {
    native.catalogListenerCalls += 1;
    native.catalogHandlers = handlers;
    return () => undefined;
  }),
}));
...
const astrometry = {
  referenceFrame: "ICRS",
  projection: "TAN",
  centerRaDegrees: 281.123456,
  centerDecDegrees: -6.123456,
  pixelScaleArcsec: 1.42,
  rotationDegrees: 0,
  rmsPixels: 0.29,
  rmsArcsec: 0.42,
  matchedStars: 73,
  ...
  indexIdentities: ["astrometry.net:index:4108:healpix:123:hpnside:4"],
...
const solvedArtifact = {
  ...
  name: "盾牌座_R_mosaic.fits",
  path: "/结果/深空 输出/ultra-fast-wbpp-run/盾牌座_R_mosaic.fits",
  detail: "final WCS",
  filter: "R",
  target: "盾牌座",
  receipt: {
    artifactId: "final-r",
    relativePath: "盾牌座_R_mosaic.fits",
    sha256: "a".repeat(64),
    sizeBytes: 4096,
    astrometry,
  },
...
].map((code) => ({ code, required: true, passed: true, artifactIds: ["final-r"], message: "ready" }));
...
beforeEach(() => {
  ...
  native.catalogHandlers = undefined;
  native.inspectError = undefined;
  native.qualityDisposition = "PASS";
  native.reviewFirst = false;
  native.cfa = false;
  native.unknownCfa = false;
  native.useMasterDark = false;
  native.catalogReady = true;
  native.catalogInspectionError = undefined;
  native.capabilityCalls = 0;
  native.solverSetupCalls = 0;
  native.dropListenerCalls = 0;
  native.pipelineListenerCalls = 0;
  native.catalogListenerCalls = 0;
  native.lightCount = 1;
  native.solverReady = true;
  native.solverError = undefined;
  native.dropHandler = undefined;
  native.platform = "macos";
  native.astapReady = false;
  native.astapScienceReady = false;
  native.inspectPaths.mockReset().mockImplementation(async () => {
    if (native.inspectError) throw native.inspectError;
    return inventory();
  });
  native.inspectCalibration.mockReset().mockImplementation(async ({ paths }: { paths: string[] }) => {
    ...
    return {
      ...
      issues: ready
        ...
        : [
            {
              code: "FLAT_MATCH_MISSING",
              severity: "ERROR",
              message: "No compatible flat for R",
              paths,
              lightGroups: [],
            },
          ],
  ...
  native.blinkMeasure
    ...
    .mockImplementation(async () =>
      inventoryBlinkManifest((await native.inspectPaths.mock.results.at(-1)!.value).assets),
    );
  ...
  native.cancelRun.mockReset().mockResolvedValue(undefined);
  native.startCatalogInstall
    .mockReset()
    .mockResolvedValue({ jobId: "catalog-1", accepted: true, catalogId: "astrometry-net-4107-4112" });
  native.cancelCatalogInstall.mockReset().mockResolvedValue(undefined);
  native.verifyCatalog.mockReset().mockImplementation(async () => {
    native.catalogReady = true;
    return { schemaVersion: 1, ok: true };
  });
  native.openProviderTerms.mockReset().mockResolvedValue(undefined);
  native.hashSources.mockReset().mockImplementation(async (paths: string[]) => ({
    entries: paths.map((path) => ({
      path,
      sourceSha256: `sha256:${(path.includes("亮场") ? "9" : path.includes("平场") ? "a" : path.includes("暗场") ? "b" : "c").repeat(64)}`,
    })),
  }));
  native.inspectQuality.mockReset().mockImplementation(async (paths: string[]) => {
    const dispositions = paths.map((_, index) =>
      native.reviewFirst && index === 0 ? ("REVIEW" as const) : native.qualityDisposition,
    );
    return {
      schemaVersion: 1 as const,
      gatePolicyDigest: `sha256:${"7".repeat(64)}`,
      workers: 2,
      counts: {
        PASS: dispositions.filter((value) => value === "PASS").length,
        REVIEW: dispositions.filter((value) => value === "REVIEW").length,
        HARD_FAIL: dispositions.filter((value) => value === "HARD_FAIL").length,
      },
      frames: paths.map((path, index) => {
        const disposition = dispositions[index];
        return {
          path,
          sourceSha256: `sha256:${index === 0 ? "9".repeat(64) : index.toString(16).padStart(64, "0")}`,
          disposition,
          decision: disposition === "PASS" ? "KEEP" : "REVIEW",
          confidence: "HIGH",
          starCount: 420,
          summary: "real gate fixture",
          previewDataUrl: disposition === "PASS" ? null : "data:image/png;base64,iVBORw0KGgo=",
          previewSha256: disposition === "PASS" ? null : `sha256:${"6".repeat(64)}`,
          evidence:
            disposition === "PASS"
              ? []
              : [
                  {
                    ...
                    message: "fixture requires review",
                  },
                ],
        };
      }),
...
describe("native product workflow", () => {
  it("imports pasted multiline paths and submits the entered output folder through the native workflow", async () => {
    ...
    fireEvent.change(screen.getByRole("textbox", { name: "文件或文件夹路径" }), {
      target: { value: "  /数据/夜晚 1\r\n/数据/校准/主平场.fit\n\n/数据/夜晚 1\n" },
    });
    await userEvent.click(screen.getByRole("button", { name: "导入路径" }));
    expect(native.inspectPaths).toHaveBeenCalledWith({
      paths: ["/数据/夜晚 1", "/数据/校准/主平场.fit"],
      roleHint: undefined,
    });
    ...
    fireEvent.change(screen.getByRole("textbox", { name: "输出文件夹路径" }), {
      target: { value: " /结果/深空 手动输出 " },
    });
    ...
    expect(native.startRun).toHaveBeenCalledWith(
      expect.objectContaining({
        outputParentDirectory: "/结果/深空 手动输出",
        reviewSelections: [],
        sources: expect.arrayContaining([
          expect.objectContaining({ role: "LIGHT", paths: ["/数据/盾牌座/亮场 01.fit"] }),
        ]),
      }),
    );
  });
  ...
  it("reopens completed screening without rerunning, retains it for calibration additions, and invalidates reimported Lights", async () => {
    ...
    native.inspectPaths.mockResolvedValueOnce({
      ...
      totalFiles: 1,
      assets: [extra],
      sources: [{ role: extra.role, paths: [extra.path], fileCount: 1, confidence: 1, needsConfirmation: false }],
    });
    await userEvent.click(screen.getByRole("button", { name: /选择文件夹$/ }));
  ...
  it("shows real calibration blockers even when all frame types are present", async () => {
    native.inspectCalibration.mockResolvedValue({
      schemaVersion: 1,
      status: "BLOCKED",
      ...
      issues: [
        {
          code: "FLAT_MATCH_MISSING",
          severity: "ERROR",
          message: "Flat filter does not match R",
          paths: ["/数据/校准/平场 R.fit"],
          lightGroups: ["R"],
        },
      ],
    });
  ...
  it("invalidates calibration readiness while a new check is pending or fails", async () => {
    ...
    native.inspectCalibration.mockImplementationOnce(
      () =>
  ...
  it("keeps mixed Raw and Master tabs separate from import role hints", async () => {
    ...
    mixed.assets.push({
      ...lightAsset("/数据/校准/master-flat.fit"),
      role: "MASTER_FLAT",
      observedAt: "2026-09-04T21:00:00Z",
    });
    mixed.sources.push({
      role: "MASTER_FLAT",
      paths: ["/数据/校准/master-flat.fit"],
      fileCount: 1,
      confidence: 1,
      needsConfirmation: false,
    });
  ...
  it("keeps imported files unchanged during quality inspection and processing", async () => {
    ...
    native.inspectQuality.mockImplementationOnce(
      () =>
        new Promise((resolve) => {
  ...
  it("starts only once and locks workflow navigation while a project is running", async () => {
    ...
    await userEvent.click(screen.getByRole("button", { name: "选择输出文件夹" }));
    await waitFor(() => expect(screen.getByRole("button", { name: /开始处理/ })).toBeEnabled());
    await userEvent.dblClick(screen.getByRole("button", { name: /开始处理/ }));
    expect(await screen.findByRole("heading", { name: "正在生成验证后的产品" })).toBeInTheDocument();
    expect(native.startRun).toHaveBeenCalledTimes(1);
    expect(
      within(screen.getByRole("navigation", { name: "处理流程" })).getByRole("button", { name: /处理/ }),
    ).toBeDisabled();
  });
  ...
  it("starts after Blink and shows the run's own screening with the result", async () => {
    render(<App />);
    await reachRunAfterBlink();
    expect(native.inspectQuality).not.toHaveBeenCalled();
    expect(native.startRun).toHaveBeenCalledWith(
      expect.objectContaining({ reviewSelections: [], runLabel: "盾牌座 Panel 1" }),
    );
    const preview = "data:image/png;base64,iVBORw0KGgo=";
    await act(async () =>
      native.handlers?.onComplete({
        jobId: "run-native-1",
        outputDirectory: "/结果/深空 输出/盾牌座-Panel-1_2026-09-19_0010",
        artifacts: [solvedArtifact],
        gate: { decision: "ready", checks: readyChecks },
        screening: {
          admitted: 61,
  ...
  it("preserves Unicode paths and shows WCS SOLVED only after the final gate", async () => {
    ...
    expect(native.startRun).toHaveBeenCalledWith(
      expect.objectContaining({
        outputParentDirectory: "/结果/深空 输出",
        projectName: "盾牌座 马赛克",
        sources: expect.arrayContaining([
          expect.objectContaining({ role: "LIGHT", paths: ["/数据/盾牌座/亮场 01.fit"] }),
        ]),
    ...
    await act(async () =>
      native.handlers?.onComplete({
        ...
        artifacts: [
          solvedArtifact,
          {
            kind: "LINEAR_RGB_FITS",
            name: "rgb.fits",
            path: "/结果/深空 输出/ultra-fast-wbpp-run/rgb.fits",
            detail: "linear RGB",
          },
          {
            kind: "RGB_PREVIEW_TIFF_16",
            name: "rgb.tiff",
            path: "/结果/深空 输出/ultra-fast-wbpp-run/rgb.tiff",
            detail: "16-bit",
          },
          {
            kind: "RGB_PREVIEW_PNG_16",
            name: "rgb.png",
            path: "/结果/深空 输出/ultra-fast-wbpp-run/rgb.png",
            detail: "16-bit",
          },
          {
            kind: "RECEIPT",
            name: "receipt.json",
            path: "/结果/深空 输出/ultra-fast-wbpp-run/receipt.json",
            detail: "auditable",
          },
        ],
        gate: { decision: "ready", checks: readyChecks },
    ...
    expect(await screen.findByText(/最终门禁 · WCS 已解算/)).toBeInTheDocument();
    expect(screen.getByText("线性 RGB FITS")).toBeInTheDocument();
    expect(screen.getByText("16-bit TIFF")).toBeInTheDocument();
    expect(screen.getByText("处理收据")).toBeInTheDocument();
    await userEvent.click(screen.getByRole("button", { name: "在 Finder 中显示 盾牌座_R_mosaic.fits" }));
    expect(native.revealOutput).toHaveBeenCalledWith(solvedArtifact.path);
  });
  ...
  it("keeps overall progress across panels and final operations until verified completion", async () => {
    ...
    const event = {
      ...
      fraction: 1,
      overallFraction: 0.22,
      scope: "panel" as const,
      panelId: "cartwheel__b",
      panelTarget: "Cartwheel",
      panelFilter: "B",
      panelIndex: 1,
      panelCount: 4,
    ...
    await act(async () =>
      native.handlers?.onProgress({
        ...event,
        panelId: "cartwheel__g",
        panelIndex: 2,
        panelFilter: "G",
    ...
    await act(async () =>
      native.handlers?.onProgress({
        ...
        state: "running",
        fraction: 0.5,
        overallFraction: 0.86,
        message: "aligning channels",
      }),
    );
    expect(screen.getByLabelText("86%")).toBeInTheDocument();
    expect(screen.getByText("通道对齐").closest("li")).toHaveClass("running");
    await act(async () =>
      native.handlers?.onProgress({ ...event, scope: "project", stageId: "publish", overallFraction: 1 }),
    );
  ...
  it("cancels the native process and never fabricates a result", async () => {
    ...
    expect(native.cancelRun).toHaveBeenCalledWith("run-native-1");
    expect(await screen.findByRole("heading", { name: "运行已安全取消" })).toBeInTheDocument();
    expect(screen.queryByText(/最终门禁 · WCS 已解算/)).not.toBeInTheDocument();
  });
  ...
  it("admits Bayer/OSC Lights and explains the colour channel processing", async () => {
    ...
    await userEvent.click(screen.getByRole("button", { name: /选择文件夹$/ }));
    await userEvent.click(await screen.findByRole("button", { name: /先看筛片结果（1 张 Light/ }));
    expect(screen.getByText("彩色相机（Bayer）亮场")).toBeInTheDocument();
    expect(screen.getByText(/RGGB Bayer 图案/)).toBeInTheDocument();
  ...
  it("uses the mono workflow without per-file confirmations when BAYERPAT is missing", async () => {
    native.unknownCfa = true;
    render(<App />);
    await reachRun();
    expect(native.hashSources).not.toHaveBeenCalled();
    expect(native.inspectCalibration).toHaveBeenCalledWith(
      expect.objectContaining({
        recipe: {
          calibration: { workflow: "mono-standard-v1", bias: "OPTIONAL", masterMetadataOverrides: [] },
    ...
    expect(native.startRun).toHaveBeenCalledWith(
      expect.objectContaining({
        recipe: expect.objectContaining({ calibrationWorkflow: "mono-standard-v1" }),
        rawFrameMetadataOverrides: [],
      }),
    );
  ...
  it("saves independent advanced dark semantics while blank metadata stays omitted and zero stays zero", async () => {
    ...
    await waitFor(() =>
      expect(native.inspectCalibration).toHaveBeenLastCalledWith(
        expect.objectContaining({
          recipe: {
            calibration: {
              ...
              masterMetadataOverrides: [
                { sourceSha256: `sha256:${"8".repeat(64)}`, binning: [1, 1], gain: 0, biasIncluded: false },
              ],
            },
            rawFrameMetadataOverrides: [],
          },
  ...
  it("surfaces inventory errors without claiming success", async () => {
    native.inspectError = new Error("HEADER_CONFLICT: role mismatch");
    render(<App />);
    await userEvent.click(screen.getByRole("button", { name: /选择文件夹$/ }));
    expect(await screen.findByText(/HEADER_CONFLICT/)).toBeInTheDocument();
  ...
  it("keeps diagnostic REVIEW evidence read-only and requires Blink before launch", async () => {
    ...
    expect(native.startRun).toHaveBeenCalledWith(
      expect.objectContaining({
        reviewSelections: [],
        selection: expect.objectContaining({ policy: "explicit-v1" }),
        blinkReview: expect.any(Object),
      }),
    );
  });
  ...
  it("requires two manually kept Lights in every target-filter panel", async () => {
    ...
    const mixed = inventory();
    mixed.assets
      .filter((asset) => asset.role === "LIGHT")
      .forEach((asset, index) => {
        asset.filter = index < 2 ? "B" : "L";
      });
    native.inspectPaths.mockResolvedValueOnce(mixed);
    render(<App />);
    await userEvent.click(screen.getByRole("button", { name: /选择文件夹$/ }));
    await userEvent.click(screen.getByRole("button", { name: /闪视筛片（4 张 Light/ }));
    await reviewChannel(/^确认 B/);
    await reviewChannel(/^确认 L/);
...
describe("Windows x64 solver route", () => {
  it("starts with a science-ready ASTAP when solve-field is absent and names the validated CPU path", async () => {
    ...
    expect(screen.getByLabelText("计算后端")).toHaveTextContent(
      "Native CPU execution · GPU acceleration not used · Windows x64 · 已验证的 CPU 路径",
    );
  ...
  it("never lets ASTAP satisfy the gate on an engine that does not report scienceReady", async () => {
    ...
    native.solverReady = false;
    native.astapReady = true;
    const doctor = await import("./bridge").then(
      (module) => module.desktopBridge.solverDoctor as unknown as ReturnType<typeof vi.fn>,
    );
    const original = doctor.getMockImplementation()!;
    doctor.mockImplementation(async () => {
      const report = await original();
      report.backends = report.backends.map((backend: { backendId: string; scienceReady?: boolean }) => {
        const { scienceReady: _ignored, ...rest } = backend;
        return rest;
      });
      return report;
    });
...
describe("result previews", () => {
  it("prefers the controller's preview data URLs over asset-protocol paths", async () => {
    ...
    const rgbPreview =
      "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAAAAAA6fptVAAAACklEQVR4nGP4DwQACfsD/Wm0a9YAAAAASUVORK5CYII=";
    await act(async () =>
      native.handlers?.onComplete({
        jobId: "run-native-1",
        outputDirectory: "D:\\astro\\out\\NGC7331_2026-09-21_2210",
        artifacts: [
          solvedArtifact,
          {
            kind: "LINEAR_RGB_FITS",
            name: "rgb.fits",
            path: "D:\\astro\\out\\NGC7331_2026-09-21_2210\\rgb.fits",
            detail: "linear RGB",
          },
          {
            kind: "MONO_PREVIEW_PNG",
            name: "盾牌座_R_mosaic.png",
            path: "D:\\astro\\out\\NGC7331_2026-09-21_2210\\previews\\盾牌座_R_mosaic.png",
            detail: "auto-stretched mono preview",
            filter: "R",
            target: "盾牌座",
            previewDataUrl: monoPreview,
          },
          {
            kind: "RGB_PREVIEW_PNG_16",
            name: "rgb.png",
            path: "D:\\astro\\out\\NGC7331_2026-09-21_2210\\rgb.png",
            detail: "16-bit display preview",
            previewDataUrl: rgbPreview,
          },
        ],
        gate: { decision: "ready", checks: readyChecks },
      }),
      ...
      native.handlers?.onComplete({
        ...
        artifacts: [
          solvedArtifact,
          {
            kind: "MONO_PREVIEW_PNG",
            name: "盾牌座_R_mosaic.png",
            path: "/结果/final/previews/盾牌座_R_mosaic.png",
            detail: "auto-stretched mono preview",
            filter: "R",
            target: "盾牌座",
          },
...
describe("native run elapsed time", () => {
  const completion = () => ({
    ...
    outputDirectory: "/结果/final",
    artifacts: [solvedArtifact],
    gate: { decision: "ready" as const, checks: readyChecks },
  });
  ...
  it("counts only this run across channels and clock changes, freezes success, and resets for a new project", async () => {
    ...
    await act(async () =>
      native.handlers?.onError({ jobId: "run-native-1", code: "LATE_ERROR", message: "late", retryable: false }),
    );
    expect(elapsed()).toHaveTextContent("01:01:03");
  ...
  it("includes launch waiting and preserves completion time when the receipt arrives later", async () => {
    ...
    native.startRun.mockImplementationOnce(
      () =>
        new Promise((resolve) => {
          accept = resolve;
        }),
    ...
    await advance(5_000);
    await act(async () =>
      accept({ jobId: "run-native-1", accepted: true, executionMode: "native", outputDirectory: "/结果/final" }),
    );
  ...
  it("freezes native failure and starts a retry from zero", async () => {
    ...
    await advance(4_700);
    await act(async () =>
      native.handlers?.onError({ jobId: "run-native-1", code: "PROCESS_FAILED", message: "failed", retryable: true }),
    );
    ...
    native.startRun.mockResolvedValueOnce({
      jobId: "run-native-2",
      accepted: true,
      executionMode: "native",
      outputDirectory: "/结果/retry",
    });
    await start();
    expect(elapsed()).toHaveTextContent("00:00:00");
    await act(async () => native.handlers?.onComplete(completion()));
  ...
  it.each(["start error", "start rejected", "final gate"])("freezes elapsed time after %s", async (failure) => {
    ...
    await act(async () => {
      if (failure === "final gate")
        native.handlers?.onComplete({ ...completion(), gate: { decision: "blocked", checks: [] } });
      else finishLaunch();
    });
  ...
  it.each([false, true])(
    "preserves the confirmed outcome and time across cancellation races (cancel rejected: %s)",
    async (cancelFails) => {
      let finishCancel!: () => void;
      native.cancelRun.mockImplementationOnce(
        () =>
          new Promise((resolve, reject) => {
      ...
      expect(
        screen.getByRole("heading", { name: cancelFails ? "每个最终通道都已写入天文坐标" : "运行已安全取消" }),
      ).toBeInTheDocument();
...
describe("native run failure", () => {
  it("stops the spinner, marks the running stage failed and shows the failure card with the evidence path", async () => {
    ...
    await act(async () =>
      native.handlers?.onProgress({ ...event, stageId: "alignment", state: "succeeded", fraction: 1 }),
    );
    ...
    expect(card).toHaveTextContent(
      "ASTROMETRY_REQUIRED: one or more filters did not produce a verified new WCS solution",
    );
...
describe("explicit catalog setup", () => {
  it("requires exact terms acceptance, streams fake-sidecar progress, and can cancel", async () => {
    ...
    render(<App />);
    await reachRecipe();
    const install = await screen.findByRole("button", { name: /下载并校验/ });
    expect(install).toBeDisabled();
    await userEvent.click(screen.getByRole("link", { name: /查看提供方条款与索引说明/ }));
    expect(native.openProviderTerms).toHaveBeenCalledWith("https://astrometry.net/doc/readme.html#getting-index-files");
    await userEvent.click(screen.getByLabelText(/我已阅读提供方说明/));
    expect(install).toBeEnabled();
    await userEvent.click(install);
    expect(native.startCatalogInstall).toHaveBeenCalledWith({
      catalogId: "astrometry-net-4107-4112",
      acceptedTermsId: "astrometry-net-index-data-2026-09",
    });
    await act(async () =>
      native.catalogHandlers?.onProgress({
        jobId: "catalog-1",
        catalogId: "astrometry-net-4107-4112",
        artifactId: "index-4108.fits",
        downloadedBytes: 50,
        sizeBytes: 100,
      }),
    );
    expect(screen.getByText(/index-4108.fits · 50.0%/)).toBeInTheDocument();
    await userEvent.click(screen.getByRole("button", { name: "取消下载" }));
    expect(native.cancelCatalogInstall).toHaveBeenCalledWith("catalog-1");
    await act(async () =>
      native.catalogHandlers?.onError({
        jobId: "catalog-1",
        catalogId: "astrometry-net-4107-4112",
    ...
    expect(screen.queryByText(/CATALOG_INSTALL_FAILED/)).not.toBeInTheDocument();
  });
  ...
  it("verifies and configures only after download completion", async () => {
    ...
    await reachRecipe();
    await userEvent.click(screen.getByLabelText(/我已阅读提供方说明/));
    await userEvent.click(screen.getByRole("button", { name: /下载并校验/ }));
    await act(async () =>
      native.catalogHandlers?.onComplete({
        jobId: "catalog-1",
        catalogId: "astrometry-net-4107-4112",
        install: { schemaVersion: 1 },
      }),
    );
    await waitFor(() => expect(native.verifyCatalog).toHaveBeenCalledWith("astrometry-net-4107-4112", true));
    expect(await screen.findByText("离线星表已校验并配置")).toBeInTheDocument();
  });
});
