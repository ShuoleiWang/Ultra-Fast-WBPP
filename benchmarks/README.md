...
[`trace_run.py`](trace_run.py) traces a **real** run: it executes any engine CLI command in-process with timers around the stages, the native kernels, the FITS/XISF I/O and the numerical steps (the spawned quality-control and registration workers install the same timers and hand their events back), samples the process's CPU utilisation and resident memory, and writes a Chrome trace-event file for [Perfetto](https://ui.perfetto.dev) plus a summary (per stage: wall, CPU of this process and of its children, cores busy; per timer: inclusive and exclusive time, calls, mean, maximum):

    .venv/bin/python benchmarks/trace_run.py --trace build/trace.json --summary build/trace-summary.json -- \
        run-project ~/Astro/Target ~/Astro/Masters/masterDark.xisf --recipe recipe.json --output ~/Astro/Results/trace --workers 8

`--profile <timer>` (repeatable) additionally runs that timer's calls under cProfile — one call at a time, so concurrent calls are sampled — and prints the merged statistics; on the NGC 7331 run it shows the per-frame normalization fit (`normalization.fit_coefficient`, 1.2 s of CPU per frame) spread over tens of thousands of small NumPy calls (per-tile medians, quantiles, clips and lattice samples), i.e. a vectorisation candidate rather than a kernel candidate. The exclusive time of a timer that runs in several Python threads is thread-wall, not CPU: a Python-level step that holds the GIL while native kernels run in other threads shows up inflated, which is exactly the contention the tracer is meant to expose. On the 61-frame NGC 7331 project (M3 Pro) the tracer found the per-row NumPy loop that applied the normalization offset grid during the integration reads (150 s of thread-wall in twelve threads for 4 s of arithmetic) — vectorised across the band, the run went from 95.7 s to 89.8 s with bit-identical masters — the registration's member estimation and full-resolution refinement running as GIL-bound threads (`register_analyses` 8.1 s; now in the stage's worker pool, 3.8 s, transforms unchanged; 82.0 s in total) — and the NumPy debayer of the one-shot-colour path (1.3 s per 26 MP frame inside the fused workers), now a native kernel (29 ms, value-identical). Remaining hot spots recorded in the trace: the Lanczos-3 warp (181 s of CPU for 61 frames, 6 exact transcendental evaluations per output pixel, at its floor without a weight table), the per-frame global-normalization fits (1.2 s of CPU per frame; process-pool fits were measured and gave nothing back, the work is CPU not GIL, and the idle workers slowed the following integration) and `solve-field` (4–6 s per filter, external).

[`master_tolerance_gate.py`](../tools/validation/master_tolerance_gate.py) compares two product sets of the same run when a change is allowed to move pixels (a different numerical evaluation, such as the deterministic Lanczos-3 weight table of kernel v3): identical NaN masks, ≥ 99.99 % of the finite pixels within 16 Float32 ulps or 0.002 σ of the baseline (whichever is larger — last-ulp changes of the registered pixels move the normalization's robust statistics by one order statistic, shifting whole tiles by ~1e-3 of the noise), at most 1e-6 of the pixels outside that bulk and each within 2 σ (rejection decisions flipping at the clip boundary), accepted-sample counts of the integration maps within 0.01 %, and — given `evaluate_masters.py` reports of both product sets against the same reference (`--evaluation baseline.json candidate.json`, repeatable) — identical per-metric PASS/WARN/FAIL statuses with the SNR gains G_4/G_8 and the depth Δm within `--evaluation-tolerance` (0.002, an order of magnitude under the evaluator's confidence intervals). The per-filter label (EQUIVALENT/INCONCLUSIVE/…) is read from the sibling `summary.md` and reported; a label that flips while the statuses and numbers are unchanged is the label rule's hard threshold crossed by a rounding and is noted, not failed. Kernel v3 versus v2 on the 61-frame NGC 7331 project: bulk 99.99998–100 %, p99.99 |Δ| ≤ 6e-3 ADU against σ ≈ 3.3–5.4 ADU, 1–5 outlier pixels per master with a maximum of 1.22 σ, identical accepted-sample counts, all 124 per-metric statuses identical, headline shifts ≤ 1e-4 (R's G_8 1.019993 → 1.020002 crosses the `|G_8 − 1| ≤ 0.02` equivalence threshold: label EQUIVALENT → INCONCLUSIVE) — and the warp 2.02 s → 1.35 s per rotated 26 MP frame on one core, the project 82.0 s → 76.2 s.
...
Reproduce it with the opt-in CMake benchmark command documented in [`native/README.md`](../native/README.md). The output path must not exist.
