...
The desktop app launches a target-suffixed, one-directory Python worker. Build it
...
```bash
python -m pip install -r packaging/worker/requirements-build.txt
python scripts/build_worker_sidecar.py \
...
Before PyInstaller runs, the builder performs a real canonical app-core v1
...
The same executable has two modes: no arguments starts the long-running worker;
`controller-plan ...` runs the one-shot canonical plan builder used by the GUI.
This keeps role discovery and `inputManifestSha256` generation inside the same
frozen implementation that validates and executes the plan.
...
the same target-triple stem. The v2 manifest records every relative file,
directory and safe in-tree symlink, every file SHA-256 and size, executable
bits, and a deterministic whole-tree digest. It also records the protocol and
...
contract is [`sidecar-manifest-v2.schema.json`](sidecar-manifest-v2.schema.json).

`scripts/stage_tauri_sidecar.py` verifies the complete tree again and stages it
create-only under `apps/desktop/src-tauri/resources/openastroflow-worker/`.
...
they continue through the platform-native worker path.
