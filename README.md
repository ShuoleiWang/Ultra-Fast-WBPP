...
- **A desktop that stays out of the way.** Import every night at once, blink and select each channel with the unusable frames pre-marked (the legacy review page stays optional), honest failure states, light and dark, English and Chinese, a reproducible icon and screenshots. → [blink screening](docs/recipes/blink-screening.md), [desktop guide](apps/desktop/README.md), [design record](docs/gui-redesign-plan.md)
...
2. **Blink and select.** Every Light is measured once (transparency, extinction, native-resolution PSF, trails, occlusion, field agreement), and the frames that are computably unusable — a moonlit or hazy night, collapsed star counts, thick cloud, trails, frames that cannot be registered — are pre-marked with the reason. A reference frame is chosen per channel and every frame is registered and normalised to it, so you blink through each channel with one shared stretch and decide frame by frame: keyboard, playback, drop a whole night, apply the flags, undo. Your decisions go into the run as an explicit selection that the receipt records with every override. Start without blinking and the legacy gate screens automatically (it cannot see a uniformly bad night, and says so). With `selection.policy: unattended-v1` in the recipe the same evidence becomes *keep*, *keep with reduced weight* or *exclude* without a review gate, checked by the counterfactual and written to `qc/selection.json`.
...
- **Selection.** The desktop's primary path is blink and select: the flags and the reference rule were fixed on one six-night campaign and pre-mark frames, you decide, and the run records the selection. A run started without blinking uses the legacy gate (PASS frames are stacked, REVIEW frames are excluded unless approved), which cannot see a uniformly bad night. The unattended policy with the counterfactual and region weights is a recipe option for the command line; its default stays `legacy-gate` until more data sets have been run.
