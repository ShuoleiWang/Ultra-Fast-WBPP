...
- **Nothing is published unverified.** Sources stay read-only, results go into a new directory, every product carries a receipt with hashes and the identifiers of everything that ran, and the desktop checks the final sky coordinates before it says "done". Validated mono reruns reproduce the same FITS bytes; RGB pixels are identical while checksum timestamps can differ.
- **Measured against WBPP, not compared by eye.** [`tools/validation/evaluate_masters.py`](tools/validation/evaluate_masters.py) scores a master against the PixInsight WBPP master of the same data across PSF, noise gain, depth, background flatness, artefacts and photometry with confidence intervals. On the reference data set historical measurements put binned noise gain at 1.01–1.03 on all four filters; full equivalence is not certified while standard gates remain unmeasured ([standard](docs/master-evaluation-standard.md)).
...
| Master quality | Historical G₈ 1.01–1.03; current evaluator fails closed on failed or unmeasured gates | the reference |
...
- **A desktop that stays out of the way.** Import every night at once, review and confirm every channel in Blink, with algorithmic quality hints and an independent reference per channel, honest failure states, light and dark, English and Chinese, a reproducible icon and screenshots. → [blink screening](docs/recipes/blink-screening.md), [desktop guide](apps/desktop/README.md), [design record](docs/gui-redesign-plan.md)
...
1. **Import together.** Drop every night at once: Lights, raw calibration frames and existing masters. Types, targets, filters and acquisition profiles come from the headers; conflicts are shown, never guessed. A folder prepared for PixInsight WBPP imports as it is: calibration is paired by WBPP's rules, each night takes its own Flats through `NIGHT_…`/`DATE_…` folders, and every compromise is shown as a warning ([calibration](docs/recipes/calibration.md#files-from-pixinsight-wbpp)). Bayer Lights are recognised and processed as colour channel groups.
2. **Blink and select.** Desktop processing requires human review: inspect every Light and confirm each channel. Quality flags are suggestions and do not automatically drop frames. Each channel gets a reference chosen from measured transparency, noise, native PSF and registration quality; its previews share alignment, photometric normalization and one stretch. Step or play, compare with the reference, keep/drop frames or a night, and undo. Only successfully displayed main previews count as viewed; unavailable previews must be retried or explicitly dropped. The controller checks the completed review against the measured session before accepting the explicit selection. The headless CLI retains `selection.policy: legacy-gate` by default and the optional `unattended-v1` recipe policy.
3. **Process locally.** Calibration, registration, normalization, integration with robust rejection, optional drizzle, and a per-channel astrometric solve run as one job with live progress on every core you have.
...
- **Mono validated on real data; one-shot colour on synthetic data only.** Bayer (RGGB/BGGR/GRBG/GBRG) Lights are calibrated as mosaics, debayered into R/G/B channel masters and combined into RGB ([recipe](docs/recipes/osc-cfa.md)), but no real OSC data set has been processed yet. Mosaics need real-data validation. The default normalization includes spatial background correction; the former LocalNormalization option has been retired.
- **Selection.** Desktop Blink review is mandatory and human decisions control admission. The flags and reference rule were developed on one six-night campaign and cannot guarantee detection of every defect. The headless default remains `legacy-gate`; unattended selection with counterfactual and region weights is a CLI recipe option.

Blink uses [complementary display diagnostics](docs/blink-display-redesign.md): a shared-scale star-detail view preserves dimming and noise, a signed background comparison keeps spatial changes visible, and corresponding original-pixel star regions separate morphology from brightness. Complete supplied Flat plus Dark/Bias calibrate the previews; missing calibration or unavailable comparisons are stated. The read-only display-study tool uses the same algorithms. The inspector wraps long filenames and calibration messages with automatic section heights and vertical scrolling.
...
| Luminance master vs PixInsight WBPP | Historical G₈ ≈ 1.01–1.02; full equivalence not certified | [evaluation standard](docs/master-evaluation-standard.md) |
