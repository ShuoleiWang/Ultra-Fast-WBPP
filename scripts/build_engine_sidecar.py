#!/usr/bin/env python3
"""Build and attest the target-suffixed Ultra-Fast WBPP Python engine sidecar."""
...
ENGINE_PACKAGING = REPO_ROOT / "packaging" / "engine"
_POLICY_MODULE_NAME = "_ufwbpp_engine_resource_policy"
_policy_spec = importlib.util.spec_from_file_location(
    _POLICY_MODULE_NAME, ENGINE_PACKAGING / "resource_policy.py"
)
if _policy_spec is None or _policy_spec.loader is None:
    raise RuntimeError("cannot load the engine resource policy")
...
MACOS14_RUNTIME_POLICY = ENGINE_PACKAGING / "macos14-runtime-libraries-v1.json"
...
def collect_versions() -> dict[str, Any]:
    ...
    for import_name in REQUIRED_COLLECTIONS:
        ...
        try:
            ...
            raise SidecarBuildError(
                "DEPENDENCY_MISSING",
                f"required engine distribution is not installed: {distribution_name}",
            ) from error
    ...
    try:
        ...
        raise SidecarBuildError(
            "PYINSTALLER_MISSING",
            "PyInstaller is not installed; install packaging/engine/requirements-build.txt "
...
def validate_doctor_report(report: object) -> dict[str, Any]:
    """Check an engine's ``doctor --json`` self-report."""

    if not isinstance(report, dict) or report.get("schemaVersion") != 1:
        raise SidecarBuildError("DOCTOR_INVALID", "engine doctor report is not a v1 JSON object")
    _safe_scalar(report.get("engineVersion"), "doctor.engineVersion")
    status = report.get("status")
    if not isinstance(status, dict) or status.get("pixelExecutionReady") is not True:
        raise SidecarBuildError("DOCTOR_INVALID", "the engine's pixel executors are not ready")
    native = report.get("nativeKernels")
    if not isinstance(native, dict) or not isinstance(native.get("loaded"), bool):
        raise SidecarBuildError("DOCTOR_INVALID", "engine doctor omitted the nativeKernels facts")
    return report
...
def run_engine_doctor(
    ...
    *,
    ...
    """Run ``<engine> doctor --json`` and validate its self-report."""

    if not command or timeout_seconds <= 0:
        raise ValueError("engine doctor requires a command and a positive timeout")
    try:
        completed = subprocess.run(
            [*command, "doctor", "--json"],
            text=True,
        ...
        raise SidecarBuildError("DOCTOR_FAILED", str(error)) from error
    if completed.returncode != 0:
        diagnostic = completed.stderr.strip()[-1000:] or "no stderr"
        raise SidecarBuildError(
            "DOCTOR_FAILED", f"engine doctor exited with {completed.returncode}: {diagnostic}"
        )
    try:
        report = json.loads(completed.stdout)
    except json.JSONDecodeError as error:
        raise SidecarBuildError("DOCTOR_INVALID", "engine doctor did not emit JSON") from error
    return validate_doctor_report(report)
...
def run_catalog_list_smoke(
    ...
    with tempfile.TemporaryDirectory(prefix="ultra-fast-wbpp-empty-catalog-") as catalog_root:
        try:
            completed = subprocess.run(
                [
                    *command,
...
def loaded_native_kernel_facts(report: Mapping[str, Any]) -> dict[str, Any]:
    """Return the doctor's ``nativeKernels`` facts of a frozen engine that loaded them.

    An engine that would silently run the NumPy fallback is a packaging
    error, not a runtime condition.
    """

    facts = dict(report["nativeKernels"])
    if facts["loaded"] is not True:
        raise SidecarBuildError(
            "NATIVE_KERNELS_MISSING",
            "the frozen engine did not load the native kernel library: "
            + str(facts.get("reason", "unknown reason")),
        )
    return facts


def run_source_doctor() -> dict[str, Any]:
    return run_engine_doctor(
        [sys.executable, str(ENGINE_PACKAGING / "launcher.py")],
...
def build_manifest(
    ...
    *,
    ...
    _safe_scalar(engine_version, "engine.version")
    payload = {
...
def validate_manifest(payload: object) -> dict[str, Any]:
    ...
    if payload["kind"] != "ufwbpp-engine-sidecar":
    ...
    if not isinstance(engine, dict) or set(engine) != {"version"}:
        raise ResourceBoundaryError("invalid engine manifest fields")
    _safe_scalar(engine["version"], "engine.version")
...
def _run_pyinstaller(stage: Path, target_triple: str) -> Path:
    ...
    environment["UFWBPP_WORKER_BASENAME"] = sidecar_stem(target_triple)
    ...
    command = [
        ...
        "--workpath",
        str(work),
        str(ENGINE_PACKAGING / "ufwbpp_engine.spec"),
    ]
    completed = subprocess.run(
        command,
        text=True,
...
def build_sidecar(
    ...
    *,
    ...
    source_version = run_source_doctor()["engineVersion"]
    run_public_tree_check()
    versions = collect_versions()
    if source_version != versions["packages"]["ufwbpp"]:
        raise SidecarBuildError(
            "VERSION_MISMATCH", "engine doctor and installed engine distribution disagree"
        )

    output.mkdir(parents=True, exist_ok=True)
    artifact_destination, manifest_destination = ensure_outputs_absent(output, target)
    with tempfile.TemporaryDirectory(prefix=".engine-build-", dir=output) as temporary_name:
        ...
        frozen_report = run_engine_doctor(frozen_command)
        if frozen_report["engineVersion"] != source_version:
            raise SidecarBuildError(
                "VERSION_MISMATCH", "source and frozen engine versions disagree"
            )
        run_catalog_list_smoke(frozen_command)
        if require_native_kernels:
            native_facts = loaded_native_kernel_facts(frozen_report)
            print(
                "frozen engine loaded native kernels "
        ...
        manifest = build_manifest(
            ...
            target,
            versions=versions,
            engine_version=frozen_report["engineVersion"],
        )
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--preflight-only",
        action="store_true",
        help="run the source doctor and public-tree gates without requiring PyInstaller",
    )
    parser.add_argument(
        "--allow-missing-native-kernels",
        action="store_true",
        help=(
            "do not fail when the frozen engine cannot load the native kernel "
