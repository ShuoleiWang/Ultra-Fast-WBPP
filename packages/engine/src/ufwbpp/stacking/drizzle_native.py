...
from ..image_io.fits import plain_header_value
...
from .integration import CalibrationError, FitsFrame
...
def drizzle_group(request: DrizzleGroupRequest) -> DrizzleGroupResult:
    ...
    reader = ThreadPoolExecutor(max_workers=1, thread_name_prefix="ufwbpp-drizzle-read")
    try:
        for frame in request.frames:
    ...
    for key, value in header_metadata.items():
        if value is not None:
            primary.header[str(key).strip().upper()] = plain_header_value(value)
    ...
    try:
        ...
        receipt: dict[str, Any] = {
            "receiptId": "sha256:" + hashlib.sha256(canonical_json(receipt_core)).hexdigest(),
            **receipt_core,
        }
...
def verify_drizzle_receipt(receipt_path: str | os.PathLike[str]) -> dict[str, Any]:
    ...
    if payload.get("receiptId") != "sha256:" + hashlib.sha256(canonical_json(core)).hexdigest():
