...
from ..lanczos_table import TAP_OFFSETS
...
def _coordinate_digest(
    ...
    digest.update(b"ultra-fast-wbpp-integration-sample-grid-v1\0")
    ...
    *,
    ...
    if reader_count == 1:
        ...
        with ThreadPoolExecutor(
            max_workers=reader_count, thread_name_prefix="ufwbpp-transient"
        ) as pool:
    ...
    *,
    ...
    try:
        with ExitStack() as stack:
            ...
            band_readers = (
                stack.enter_context(
                    ThreadPoolExecutor(max_workers=read_workers, thread_name_prefix="ufwbpp-band")
                )
                if read_workers > 1
                else None
            )
