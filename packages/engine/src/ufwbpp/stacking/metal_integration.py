...
class NativeMetalExecutor:
    ...
    def __init__(
        ...
        *,
        ...
        candidates = candidate_library_paths(library_path)
        if not candidates:
        ...
        self._bind()
        if int(self._library.ufwbpp_native_abi_version()) != NATIVE_ABI_VERSION:
            raise MetalIntegrationError("native ABI version differs from Python adapter")
        error = ctypes.create_string_buffer(1024)
        available = ctypes.c_uint32(0)
        status = int(
            self._library.ufwbpp_native_metal_available_v1(
        ...
        status = int(
            self._library.ufwbpp_native_metal_executor_create_v1(
    ...
    def _bind(self) -> None:
        ...
        required = (
            ...
            "ufwbpp_native_cpu_linear_fit_v1",
            "ufwbpp_native_cpu_masked_weighted_v1",
            "ufwbpp_native_metal_available_v1",
            "ufwbpp_native_metal_executor_create_v1",
            "ufwbpp_native_metal_executor_destroy_v1",
            "ufwbpp_native_metal_linear_fit_v1",
            "ufwbpp_native_metal_masked_weighted_v1",
        )
        ...
        library.ufwbpp_native_cpu_linear_fit_v1.argtypes = common
        library.ufwbpp_native_cpu_linear_fit_v1.restype = ctypes.c_int
        masked_common = [
        ...
        library.ufwbpp_native_cpu_masked_weighted_v1.argtypes = masked_common
        library.ufwbpp_native_cpu_masked_weighted_v1.restype = ctypes.c_int
        library.ufwbpp_native_metal_available_v1.argtypes = [
            ctypes.POINTER(ctypes.c_uint32),
            ctypes.POINTER(ctypes.c_char),
            ctypes.c_size_t,
        ]
        library.ufwbpp_native_metal_available_v1.restype = ctypes.c_int
        library.ufwbpp_native_metal_executor_create_v1.argtypes = [
            ctypes.c_char_p,
            ctypes.POINTER(ctypes.c_void_p),
            ctypes.POINTER(ctypes.c_char),
            ctypes.c_size_t,
        ]
        library.ufwbpp_native_metal_executor_create_v1.restype = ctypes.c_int
        library.ufwbpp_native_metal_executor_destroy_v1.argtypes = [ctypes.c_void_p]
        library.ufwbpp_native_metal_executor_destroy_v1.restype = None
        library.ufwbpp_native_metal_linear_fit_v1.argtypes = [
            ctypes.c_void_p,
            ctypes.POINTER(_RequestV1),
            ctypes.POINTER(_OutputV1),
        ...
        library.ufwbpp_native_metal_linear_fit_v1.restype = ctypes.c_int
        library.ufwbpp_native_metal_masked_weighted_v1.argtypes = [
            ctypes.c_void_p,
            ctypes.POINTER(_MaskedRequestV1),
            ctypes.POINTER(_OutputV1),
            ctypes.POINTER(_StatsV1),
        ...
        library.ufwbpp_native_metal_masked_weighted_v1.restype = ctypes.c_int
    ...
    def close(self) -> None:
        ...
        if handle is not None and handle.value:
            self._library.ufwbpp_native_metal_executor_destroy_v1(handle)
            handle.value = None
    ...
    def run_tile(
        ...
        *,
        ...
        status = int(
            self._library.ufwbpp_native_metal_linear_fit_v1(
                self._handle,
        ...
        if compare_cpu:
            ...
            error.value = b""
            status = int(
                self._library.ufwbpp_native_cpu_linear_fit_v1(
                    ctypes.byref(request),
    ...
    def run_masked_tile(
        ...
        *,
        ...
        stats.struct_size = ctypes.sizeof(_StatsV1)
        status = int(
            self._library.ufwbpp_native_metal_masked_weighted_v1(
                self._handle,
                ctypes.byref(request),
                ctypes.byref(output),
                ctypes.byref(stats),
        ...
        if compare_cpu:
            ...
            error.value = b""
            status = int(
                self._library.ufwbpp_native_cpu_masked_weighted_v1(
                    ctypes.byref(request),
                    ctypes.byref(cpu_output),
                    error,
                    ctypes.sizeof(error),
...
def metal_executor_available(hardware: HardwareProfile) -> bool:
    """Whether this host can create the native Metal executor.

    Only a real executor proves it: the library may lack the Metal ABI, the
    device may be absent (a virtual machine) or the shader may fail to compile.
    """
...
def integrate_registered_group(
    ...
    *,
    ...
    map_temporaries = {
        name: temporary_output(path) for name, path in map_destinations.items()
    }
    try:
        owns_executor = False
        ...
        try:
            ...
            with executor_context as active_executor, ExitStack() as stack:
                ...
                with ExitStack() as output_stack:
                    ...
                    with ThreadPoolExecutor(
                        ...
                        thread_name_prefix="ufwbpp-metal",
                    ) as gpu_pool, ThreadPoolExecutor(
                        max_workers=cpu_workers_used,
                        thread_name_prefix="ufwbpp-mask",
                    ) as cpu_pool:
            ...
            for name, map_destination in map_destinations.items():
                atomic_publish_file(map_temporaries[name], map_destination)
            return IntegrationResult(
                ...
                execution={
                    ...
                    "nativeLibrary": {
                        ...
                        "sha256": sha256_digest(executor.library_path),
                    },
                    "deviceName": device_name,
