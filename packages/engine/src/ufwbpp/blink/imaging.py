"""Blink preview primitives: channel stretches, preview calibration, warping onto
the reference, encoding, and the frame spec/result records.

Both renderers build on these: :mod:`.previews` (the per-frame filmstrip and
zoom) and :mod:`.diagnostic_render` (the complementary diagnostics).
"""
...
if TYPE_CHECKING:
    from .diagnostic_render import DiagnosticFrameSpec
import warnings
...
def master_preview(path: str, long_edge: int) -> np.ndarray:
...
def calibrate_linear(image: np.ndarray, calibration: PreviewCalibration | None) -> tuple[np.ndarray, bool]:
    ...
    if calibration is None:
        return image, False
    try:
        flat = master_preview(calibration.flat_path, calibration.long_edge)
    except Exception:
        return image, False
    ...
    if calibration.pedestal_path is not None:
        try:
            pedestal = master_preview(calibration.pedestal_path, calibration.long_edge)
        except Exception:
            pedestal = None
...
def encode_image(pixels: np.ndarray, *, format_name: str, quality: int) -> bytes:
    encoded = BytesIO()
