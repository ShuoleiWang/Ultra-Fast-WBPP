...
from .diagnostics import DISPLAY_ALGORITHM, DisplayReference, choose_display_reference, diagnostic_preview, local_noise
from .imaging import FramePreviewResult, PreviewCalibration, calibrate_linear, master_preview, warp_to_reference, write_new
from .native_crops import native_atlas, star_positions

if TYPE_CHECKING:
    from .imaging import FramePreviewSpec
...
def prepare_diagnostic_specs(
    evidence, results, measurements, linear_paths, session, request, long_edge, *, preview_calibration, preview_specs
):
    # The session passes its calibration and spec builders in, so this
    # extension depends on the previews module only and not on the session.
    _preview_calibration, _preview_specs = preview_calibration, preview_specs
    ...
    for index, frame in enumerate(evidence.inputs):
        ...
        if calibration is not None and calibration.pedestal_path is not None:
            try:
                complete = all(master_preview(p, long_edge).shape == data.shape for p in (calibration.flat_path, calibration.pedestal_path))
...
def render_diagnostic_frame(spec: FramePreviewSpec) -> FramePreviewResult:
    ...
    write_new(Path(spec.zoom_path), zoom)
    write_new(Path(spec.filmstrip_path), film)
