...
from ..integrity import canonical_json, canonical_json_document
...
def manifest_search_paths(
    ...
    *,
    ...
    paths: list[Path] = []
    configured = env.get("UFWBPP_CATALOG_MANIFEST_DIR")
    if configured:
        paths.append(Path(configured).expanduser().absolute())
    # The checked manifests are package data: the same path in the source
    # tree, an installed wheel and the frozen engine.
    paths.append(Path(__file__).resolve().parent / "catalog_manifests")
    unique: list[Path] = []
    for path in paths:
...
def _write_resume_state(
    ...
    payload = canonical_json_document(expected)
...
def _set_resume_etag(state_path: Path, state: Mapping[str, Any], entity_tag: str | None) -> dict[str, Any]:
    ...
    _replace_owned_file(state_path, canonical_json_document(updated))
    return updated
...
def write_installed_set_receipt(
    ...
    identity = _sha256_bytes(canonical_json(core))
    receipt = {
        **core,
    ...
    _atomic_create(path, canonical_json_document(receipt))
