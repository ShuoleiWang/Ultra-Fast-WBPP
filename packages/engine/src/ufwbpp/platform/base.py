...
native library's file name, ...) asks ``ufwbpp.platform.current()``
...
def resolve_data_root(preferred: Path, legacy: Path) -> Path:
    """Return ``preferred``, or ``legacy`` when only that one exists.

    Installations made under the project's former name keep working in place:
    installed catalogs record their absolute location (``astrometry.cfg`` and
    the installed-set receipt), so an existing root is used, never moved.
    """
...
def reconfigure_utf8_stdio(
    ...
    Results, receipts and progress messages are UTF-8 in both
