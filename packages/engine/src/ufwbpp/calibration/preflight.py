...
from .policy import MONO_STANDARD, workflow_receipt, resolve_dark_bias, bias_from_header, can_omit_bias, conflicting_profile_fields
...
def inspect_calibration(paths: list[str], recipe: Recipe | None = None) -> dict[str, Any]:
    ...
    planned_issues, _ = calibration_match_issues(effective_inventory, effective_recipe)
    ...
    for filter_name, flat_set in flat_sets.items():
        ...
        # WBPP's DATE/NIGHT/SESSION grouping keywords in folder names
        # (DATE_0322/) mark which night each Flat belongs to.
        ...
        if master_count > 1 or (master_count and master_count != len(flat_set)):
            if master_count > 1 and len(sessions) > 1:
                issue("FLAT_SOURCE_AMBIGUOUS", f"Filter {filter_name} has one MasterFlat per night or session ({', '.join(sessions)}), as WBPP's grouping keywords arrange them. Per-night Flats are not supported yet: process each night as its own project, or supply one MasterFlat for the filter.", affected=tuple(flat_set))
            else:
