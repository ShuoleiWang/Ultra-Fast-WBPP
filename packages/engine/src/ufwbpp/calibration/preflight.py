...
from ..planning import (
    base_compatible,
    calibration_match_issues,
    dark_compatible,
    effective_calibration_assets,
    flat_compatible,
    match_inventory_calibration,
)
from ..recipe import Recipe, Requirement
from .policy import MONO_STANDARD, workflow_receipt, resolve_dark_bias, bias_from_header, can_omit_bias, conflicting_profile_fields, unknown
...
def inspect_calibration(paths: list[str], recipe: Recipe | None = None) -> dict[str, Any]:
    ...
    assets, override_issues, _ = effective_calibration_assets(inventory, recipe)
    ...
    dark_semantics: dict[str, bool] | None = None
    if workflow == MONO_STANDARD:
        ...
        dark_semantics = {
            asset.path: bool(
                resolve_dark_bias(
                    master_semantics.get(digest(asset.path)) if master_semantics else None,
                    bias_from_header(probe_frame_metadata(Path(asset.path)).header),
                    workflow,
                )
            )
            for asset in assets
            if asset.role is AssetRole.MASTER_DARK and asset.status is AssetStatus.READY
        }
    planned_issues, _ = calibration_match_issues(effective_inventory, effective_recipe, dark_semantics)
    ...
    match = match_inventory_calibration(assets, effective_recipe, dark_semantics) if workflow == MONO_STANDARD else None
    if match is not None:
        # A flat group integrated from frames of several nights, or applied to
        # Lights of other nights, without a grouping keyword that says so.
        for key, group in sorted(match.groups["FLAT"].items()):
            members = [asset for asset in calibration if asset.path in set(group.members)]
            paired_lights = [light for light in lights if match.lights.get(light.path) is not None and match.lights[light.path].flat == key]
            dates = {asset.observed_at[:10] for asset in (*members, *paired_lights) if asset.observed_at}
            if len(dates) > 1 and not group.traits.keywords:
                issue("CAPTURE_SESSION_NOT_VERIFIED", f"Flat group {key}: frames of several nights ({', '.join(sorted(dates))}) share one master without a grouping keyword (NIGHT_1, SESSION_2, ...); confirm that dust, rotation and optics did not change.", severity="WARNING", affected=tuple(members))
        for dark in darks:
        ...
        # Lights of one target and filter taken with other settings are
        # stacked together, as WBPP does; the difference is reported.
        stacks: dict[tuple[str, str], list[FrameAsset]] = {}
        for light in lights:
            stacks.setdefault((light.target, light.filter_name), []).append(light)
        for (target, filter_name), members in sorted(stacks.items()):
            differing = [
                label
                for label, name in (("camera", "camera"), ("gain", "gain"), ("offset", "offset"), ("readout mode", "readout_mode"))
                if len({str(value) for member in members if not unknown(value := getattr(member, name))}) > 1
            ]
            if differing:
                issue("LIGHT_SETTINGS_DIFFER", f"The {filter_name} Lights of {target} were taken with different {', '.join(differing)}; they are stacked together.", severity="WARNING", affected=tuple(members), groups=tuple(sorted({member.group_id for member in members})))
    else:
        ...
        if reference:
            incompatible = tuple(asset for asset in (*calibration, *lights) if not base_compatible(reference[0], asset, workflow))
            if incompatible:
        ...
        if darks:
            unmatched = tuple(light for light in lights if not any(dark_compatible(light, dark, workflow) for dark in darks))
            if unmatched:
        ...
        for asset in calibration:
            if asset.role in {AssetRole.FLAT, AssetRole.MASTER_FLAT}:
                # One set per filter and grouping-keyword values: WBPP's
                # NIGHT_1/SESSION_2 folders give each night its own Flats.
                keywords = "".join(f";{name}={value}" for name, value in asset.grouping_keywords)
                flat_sets.setdefault(asset.filter_name.strip().upper() + keywords, []).append(asset)
        for filter_name, flat_set in flat_sets.items():
            master_count = sum(asset.role is AssetRole.MASTER_FLAT for asset in flat_set)
            sessions = sorted({session for session in session_keywords_of([asset.path for asset in flat_set]) if session})
            ...
            for flat in raw_flats:
                ...
                if exact_darks and not all(dark_compatible(flat, dark, workflow) for dark in exact_darks):
            ...
            if not master_count and len(sessions) > 1:
                issue("CAPTURE_SESSION_NOT_VERIFIED", f"Filter {filter_name}: the raw Flats come from several nights or sessions ({', '.join(sessions)}). They are combined into one master for the filter, while WBPP would calibrate each night with its own Flats; confirm that dust, rotation and optics did not change.", severity="WARNING", affected=tuple(flat_set))
            elif master_count or len(flat_dates | light_dates) > 1 or any(not asset.observed_at for asset in flat_set):
    ...
    for group_id, members in sorted(grouped.items()):
        ...
        for label, raw_role, master_role, compatible in (
            ("FLAT", AssetRole.FLAT, AssetRole.MASTER_FLAT, flat_compatible),
            ("DARK", AssetRole.DARK, AssetRole.MASTER_DARK, dark_compatible),
            ("BIAS", AssetRole.BIAS, AssetRole.MASTER_BIAS, base_compatible),
        ):
            if match is not None:
                # The groups WBPP's pairing chose for this group's Lights
                # (one per night when grouping keywords name the nights).
                keys = sorted({key for light in members if (pairing := match.lights.get(light.path)) is not None and (key := getattr(pairing, label.lower())) is not None})
                chosen = [match.groups[label][key] for key in keys]
                matches[label] = {
                    "rawCount": sum(len(group.members) for group in chosen if not group.supplied_master),
                    "masterCount": sum(1 for group in chosen if group.supplied_master),
                    "groups": keys,
                }
                continue
