...
def _ready_registry():
    """A registry whose pixel stages and solver all report ready (no probes)."""
    ...
    class ReadySolver:
        ...
        def solve(self, request: object) -> SolverResult:
            return SolverResult(
                ...
                error="the synthetic run owns this test boundary",
            )
...
def test_cli_run_with_selection_file_on_a_synthetic_project(
    ...
    import ufwbpp.runtime as runtime_module
    from ufwbpp import cli
    ...
    # the fake solver.
    monkeypatch.setattr(runtime_module, "default_registry", _ready_registry)
