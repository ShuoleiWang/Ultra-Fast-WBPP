...
`ultra-fast-wbpp` is the engine of Ultra-Fast WBPP (Python package `ufwbpp`). The desktop and headless users call the same command line: `doctor` probes hardware, kernels and solvers; `inventory`, `calibration-check`, `quality-check` and `blink-measure` inspect a project; `run` and `run-project` produce verified, plate-solved masters; `catalog` manages the offline solver indexes. Every command writes JSON to stdout, progress to stderr with `--progress-json`, and a structured error with a stable `code` on failure.
...
FITS and supported XISF pixel inputs share the same calibration path. XISF is first decoded through a size-, XML-, codec-, and working-set-bounded private FITS staging bridge; inventory support alone never implies executable pixel support. The default stellar-scale and spatial-background normalization is the supported route. Legacy recipes enabling LocalNormalization are rejected explicitly. See the root recipe guide for the exact fail-closed boundary and content-bound metadata overrides for legacy masters.
...
## How the desktop calls the engine

The desktop bundles the engine as the frozen `ufwbpp-engine` sidecar and runs
one command per operation. Its capability probe is `doctor --json` (the fields
it reads are `engineVersion`, `tuning.profileId` and
`status.{pixelExecutionReady, solverReady, drizzleReady, metalReady}`); a run
is `run-project --request-json <file> --progress-json`, whose result JSON on
stdout the desktop re-verifies before it reports success. Backend truth
boundaries are documented in [`docs/backend-contracts.md`](docs/backend-contracts.md).

## Hardware policy

- Every Apple-silicon Mac (`arm64`/`aarch64`) targets the generic CPU profile; `doctor` reports `metalReady` only after a real native executor/device/ABI probe succeeds.
...
- Windows x64 runs the `windows-x86-64-cpu-v1` tuning profile (validated on one Ryzen 7 5800H laptop and in hosted CI; see `docs/windows.md`); there is no GPU compute on Windows and the installer is not yet signed.
...
Final ordinary Light integration accepts `PipelineParameters(ordinary_integration_backend=...)` with `auto`, `portable-cpu`, `generic-apple-metal`, or `m3-pro-tuned`. `auto` selects the generic Metal path on Apple Silicon and the tuned path only on an identified M3 Pro with sufficient memory. The native library is discovered from the installed package, `UFWBPP_NATIVE_LIBRARY`, or a source-tree build; a missing/incompatible library, unavailable Metal device, resource rejection, or failed CPU↔GPU numerical gate reruns the complete group on CPU and records the reason. No fallback truncates input frames.
