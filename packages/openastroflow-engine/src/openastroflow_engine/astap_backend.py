...
``PLTSOLVD=T`` marker, a stable ``.wcs`` artifact, and the numerical validator in
``solver.py`` must all agree before a solved FITS file is published.
"""
...
ADAPTER_VERSION = "astap-process-v3"
_CAPABILITIES = (
    "seed-hints",
    "celestial-wcs",
...
class ExecutableProbe:
    ...
    def serializable(self) -> dict[str, Any]:
        ...
        sanitized = _share_safe_evidence(value)
...
def discover_astap(
    ...
    *,
    ...
    for key in ("OPENASTROFLOW_ASTAP", "ASTAP_PATH"):
...
def _regular_identity(path: Path, *, max_bytes: int | None = None) -> _FileIdentity:
    try:
        before = path.lstat()
...
def _copy_source_to_stage(source: Path, destination: Path) -> tuple[_FileIdentity, _FileIdentity]:
...
def _same_stat(path: Path, identity: _FileIdentity) -> bool:
    try:
...
def _require_unchanged_identity(
    ...
    *,
    ...
    after = _regular_identity(path, max_bytes=max_bytes)
    if after != identity:
...
class SolverProcessRuntime:
    ...
    def __init__(
        ...
        *,
        ...
        resolved = _candidate_path(executable)
        if resolved is None:
    ...
    def executable_identities(self) -> tuple[_FileIdentity, ...]:
        identities = [_regular_identity(Path(self.executable).resolve(strict=True))]
        for argument in self.executable_args:
            path = Path(argument)
            if path.is_file():
                identities.append(_regular_identity(path))
    ...
    def run(
        ...
        *,
        ...
        log_stem: str,
    ) -> tuple[_ProcessOutcome, tuple[_FileIdentity, ...]]:
        ...
        try:
            ...
            if process is not None:
                _kill_lingering_posix_group(process)
                for thread in capture_threads:
    ...
    def _write_local_diagnostic(
        ...
        *,
        ...
        payload = {
            ...
            "argv": [_redact_diagnostic_text(item, secrets) for item in outcome.argv],
            "stdoutTail": _redact_diagnostic_text(outcome.stdout_tail, secrets),
            "stderrTail": _redact_diagnostic_text(outcome.stderr_tail, secrets),
        }
...
def _redact_diagnostic_text(value: str, secrets: set[str]) -> str:
    redacted = value
...
def _receipt(payload: dict[str, Any]) -> dict[str, Any]:
    value = _share_safe_evidence(payload)
...
def _share_safe_evidence(value: Any) -> Any:
    ...
    if isinstance(value, Mapping):
        return {
            str(key): _share_safe_evidence(item)
            for key, item in value.items()
            if str(key) not in _PRIVATE_RECEIPT_KEYS
        }
    if isinstance(value, (list, tuple)):
        return [_share_safe_evidence(item) for item in value]
...
def _wcs_header_sha256(header: Mapping[str, Any] | fits.Header) -> str:
...
def verify_solver_execution_result(result: SolverResult) -> bool:
    ...
    try:
        ...
        if claimed_header != f"sha256:{_wcs_header_sha256(result.header)}":
            return False
        ...
        current = _regular_identity(Path(result.output_path))
        return (
...
def _read_control_text(path: Path) -> tuple[str, _FileIdentity]:
    identity = _regular_identity(path, max_bytes=_MAX_CONTROL_FILE_BYTES)
    try:
        text = path.read_text(encoding="utf-8", errors="replace")
    except OSError as error:
        raise SolverExecutionError("ARTIFACT_READ_FAILED", str(error)) from error
    _require_unchanged_identity(path, identity, max_bytes=_MAX_CONTROL_FILE_BYTES)
    return text, identity


def _read_wcs_header(path: Path) -> tuple[fits.Header, _FileIdentity]:
    identity = _regular_identity(path, max_bytes=_MAX_CONTROL_FILE_BYTES)
    errors: list[str] = []
    for loader in (
...
def _wcs_only_header(header: fits.Header) -> fits.Header:
    try:
...
def _is_wcs_keyword(keyword: str) -> bool:
    upper = keyword.upper()
...
def _publish_solved_copy(input_path: Path, output_path: Path, wcs_header: fits.Header) -> _FileIdentity:
    output_parent = output_path.expanduser().resolve(strict=False).parent
    output_parent.mkdir(parents=True, exist_ok=True)
    if output_path.exists() or output_path.is_symlink():
        raise SolverExecutionError("OUTPUT_EXISTS", f"refusing to replace existing output: {output_path}")
    wcs_only = _wcs_only_header(wcs_header)
    temporary_path: Path | None = None
    try:
        with fits.open(input_path, mode="readonly", memmap=False) as hdul:
            for key in list(hdul[0].header):
                upper = key.upper()
                if upper in wcs_only or _is_wcs_keyword(upper):
        ...
        temporary_shape = _input_shape(temporary_path)
        ...
        temporary_identity = _regular_identity(temporary_path)
        ...
        try:
            published = _regular_identity(output_path)
...
def _probe_astap_runtime(runtime: SolverProcessRuntime, timeout_seconds: float) -> ExecutableProbe:
    try:
        with tempfile.TemporaryDirectory(prefix="openastroflow-astap-probe-") as raw:
        ...
        ready = not outcome.timed_out and outcome.spawn_error is None and not missing
        return ExecutableProbe(
            ...
            error_code=None if ready else ("PROBE_TIMEOUT" if outcome.timed_out else "CAPABILITY_PROBE_FAILED"),
            message=None if ready else (f"ASTAP help is missing: {', '.join(missing)}" if missing else "ASTAP probe failed"),
            evidence={
                "process": outcome.serializable(),
...
def probe_astap(
    ...
    *,
    ...
    return _probe_astap_runtime(runtime, timeout_seconds)
...
class AstapSolverBackend:
    ...
    def __init__(
        ...
        *,
        ...
        path = discover_astap(executable, environment={**os.environ, **self.environment})
        ...
        if path is None:
            ...
            try:
                ...
                self.probe = _probe_astap_runtime(self.runtime, probe_timeout_seconds)
            except (ValueError, SolverExecutionError) as error:
        ...
        return BackendDescriptor(
            ...
            metadata={
                ...
                "scientificQualityEvidence": {
                    "status": "unavailable",
                    "strictE2EBehavior": "try-next-backend",
                    "reason": (
                        "no documented catalog-correspondence table with recomputable match/RMS "
                        "metrics or app-managed catalog byte identity; diagnostic only"
                    ),
                },
            },
        )
    ...
    def solve(self, request: SolveRequest) -> SolverResult:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(
                prefix="openastroflow-astap-", dir=self.staging_root
            ) as raw_stage:
                ...
                if request.field_of_view_degrees is not None:
                    arguments.extend(("-fov", f"{request.field_of_view_degrees:.12g}"))
                ...
                if marker is None or marker.group(1).strip().upper() not in {"T", "TRUE", "1", "Y", "YES"}:
                    return _failure(
                        ...
                        evidence={**base_evidence, "outputs": {"ini": ini_identity.serializable(expose_path=False)}},
                    )
                try:
                ...
                if not validation.valid:
                    return _failure(
                        ...
                        evidence={
                            **base_evidence,
                            "outputs": {
                                "ini": ini_identity.serializable(expose_path=False),
                ...
                evidence = _receipt(
                    {
                        ...
                        "outputs": {
                            "ini": ini_identity.serializable(expose_path=False),
                            "wcs": wcs_identity.serializable(expose_path=False),
                            "published": published.serializable(expose_path=False),
                        },
                        "wcsValidation": validation.serializable(),
                        ...
                        "astrometricQuality": {
                            "status": "UNAVAILABLE",
                            "reason": (
                                "ASTAP's documented .ini/.wcs contract does not expose a "
                                "catalog correspondence table with independently recomputable "
                                "unique-match and RMS evidence"
                            ),
                        },
                    }
                )
