...
def _pixel_staging_candidates(index_width: int, stem: str, filter_token: str) -> list[str]:
    ...
    suffix for Bayer Lights), locally normalized frames sit two directories
    down and the integration keeps its rejection maps in ``.work``.  The
    projection keeps every variant because the parameters that select them
    (materialized calibrated Lights, CFA, local normalization) are decided
    later than this check.
    """
    ...
    return [
        ...
        f"local-normalization/{filter_token}/frames/{index}_normalized.fits",
