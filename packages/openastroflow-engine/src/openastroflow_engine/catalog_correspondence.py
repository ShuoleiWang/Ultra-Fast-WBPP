...
from .solver import AstrometricQuality, SolverIndexArtifact, wcs_parity
...
def detect_field_stars(
    ...
    from openastroflow_registration.pipeline import DetectionConfig, detect_stars
...
def _canonical_json(value: Any) -> bytes:
    # The same canonical form the astrometry.net adapter hashes for its
    # catalog identity, so identical matched rows give identical identities.
    return json.dumps(
...
def verify_solution(
    *,
    ...
    catalog_identity = hashlib.sha256(
        _canonical_json({"indexes": list(index_identities), "matchedCatalogRows": catalog_row_records})
    ).hexdigest()
    ...
    correspondence_sha256 = _write_artifact(correspondence_path, _canonical_json(correspondence_payload))
    _write_artifact(match_path, _canonical_json(match_payload))
    quality = AstrometricQuality(
