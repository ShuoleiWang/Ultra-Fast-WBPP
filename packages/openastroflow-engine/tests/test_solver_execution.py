...
import openastroflow_engine.astap_backend as astap_backend_module
...
from openastroflow_engine.solver import SolveRequest, SolverStatus, validate_solver_result
...
def test_astrometry_config_discovery_uses_app_environment_without_probe_args(
    ...
    config.parent.mkdir()
    config.write_text("inparallel\n", encoding="ascii")
    environment = {"OPENASTROFLOW_ASTROMETRY_CONFIG": str(config)}
...
@pytest.mark.skipif(
    os.environ.get("OAF_RUN_REAL_ASTROMETRY") != "1",
...
def test_real_astrometry_fixture_passes_public_adapter_quality_gate(tmp_path: Path) -> None:
    fixture_value = os.environ.get("OAF_REAL_ASTROMETRY_FIXTURE")
    config_value = os.environ.get("OAF_ASTROMETRY_CONFIG")
    if not fixture_value or not config_value:
        pytest.skip("set OAF_REAL_ASTROMETRY_FIXTURE and OAF_ASTROMETRY_CONFIG")
