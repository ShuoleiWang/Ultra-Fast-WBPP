...
SEED = 0
BOOTSTRAP = 300
EVALUATION_VERSION = "native-grid-summary-gates-v2"
...
def absolute_interval(lo: float, hi: float, center: float = 1.0) -> tuple[float, float]:
    """Transform an interval through abs(x-center), including a crossing."""
    if not (math.isfinite(lo) and math.isfinite(hi)):
        return float("nan"), float("nan")
    return (0.0 if lo <= center <= hi else min(abs(lo - center), abs(hi - center)),
            max(abs(lo - center), abs(hi - center)))
...
def evaluate_pair(filter_name: str, ours_path: str, pi_path: str, out_dir: Path) -> dict[str, Any]:
    ...
    report: dict[str, Any] = {"evaluationVersion": EVALUATION_VERSION, "filter": filter_name, "ours": ours_path, "pi": pi_path, "metrics": [],
                              "info": {}}
    ...
    metric("photometry", "linearity slope s (log flux pi vs ours)", 1.0, float(slope), abs(slope - 1), 0.01,
           *absolute_interval(s_lo, s_hi), note=f"n={int(lin_sel.sum())}")
    q_bins = {}
    worst_q = 0.0
    ...
    # Unimplemented standard gates are explicit, not silently assumed to pass.
    report["info"]["unmeasured_standard_gates"] = [
        "common-footprint area ratio", "common/own STF agreement", "q_LSB",
    ]
...
def filter_verdict(report: dict[str, Any]) -> tuple[str, bool, bool, bool]:
    """Apply the validity gate before aggregating the measured science gates.

    Missing faint/LSB flux evidence cannot establish BETTER. Historical
    reports remain readable, but never turn absent measurements into PASS.
    """
    info = report["info"]
    metrics = [m for m in report["metrics"] if m["verdict"]]
    families = {f: [m["status"] for m in metrics if m["family"] == f]
                for f in ("psf", "background", "artefacts", "photometry", "geometry")}
    visual = [s for f in ("psf", "background", "artefacts", "geometry") for s in families[f]]
    not_worse = bool(visual) and "FAIL" not in visual and visual.count("WARN") <= 3
    sci = bool(families["photometry"]) and "FAIL" not in families["photometry"]
    psf_pass = bool(families["psf"]) and all(s == "PASS" for s in families["psf"])
    flux_pass = all(any(m["metric"].startswith(name) and m["status"] == "PASS" for m in metrics)
                    for name in ("q_faint", "q_LSB"))
    g8, g4, dm = info["noise"]["G"]["8"], info["noise"]["G"]["4"], info["depth"]
    better = g8[1] > 1.02 and g4[1] > 1.00 and dm["ci"][0] >= 0.02 and psf_pass and flux_pass
    alignment = info["alignment"]
    valid = (info.get("valid_alignment") is True and alignment["matched"] >= 200
             and math.isfinite(alignment["rms"]) and alignment["rms"] <= 0.10)
    if not valid:
        return "INCONCLUSIVE", False, False, False
    if any(m["status"] == "FAIL" for m in metrics):
        return "WORSE", not_worse, sci, False
    if any(m["status"] not in {"PASS", "WARN"} for m in metrics):
        return "INCONCLUSIVE", not_worse, sci, False
    if "unmeasured_standard_gates" not in info or info["unmeasured_standard_gates"]:
        return "INCONCLUSIVE", not_worse, sci, False
    if better and not_worse and sci:
        return "BETTER", not_worse, sci, better
    if not_worse and sci and (g8[1] <= 1.0 <= g8[2] or abs(g8[0] - 1) <= 0.02):
        return "EQUIVALENT", not_worse, sci, better
    return "INCONCLUSIVE", not_worse, sci, better


def cross_channel_status(row: dict[str, Any]) -> str:
    values = [*row["median"], row["rms"], row["worstZone"]]
    if row["matched"] < 10 or not all(math.isfinite(v) for v in values):
        return "INCONCLUSIVE"
    if math.hypot(*row["median"]) <= 0.05 and row["rms"] <= 0.10 and row["worstZone"] <= 0.10:
        return "PASS"
    return "WARN" if max(math.hypot(*row["median"]), row["rms"], row["worstZone"]) <= 0.20 else "FAIL"
...
def summarize(reports: list[dict[str, Any]], cross: list[dict[str, Any]], out_dir: Path) -> str:
    ...
    for rep in reports:
        ...
        g4 = info["noise"]["G"]["4"]
        dm = info["depth"]
        verdict, not_worse, sci, better_snr = filter_verdict(rep)
        overall.append(verdict)
        missing = info.get("unmeasured_standard_gates")
        if missing is None:
            lines.append("Standard-gate coverage is unrecorded in this historical report.")
        elif missing:
            lines.append("Unmeasured standard gates: " + ", ".join(missing) + ".")
        lines.append("")
    ...
    for c in cross:
        ...
        lines.append("|---|---|---|---|---|")
        for r in c["pairs"]:
            st = cross_channel_status(r)
            if c["label"] == "ours" and st != "PASS":
                overall.append("WORSE" if st == "FAIL" else "INCONCLUSIVE")
    ...
    lines.insert(2, f"**Run verdict: {run_verdict}** (filter and candidate cross-channel gates)")
    lines.insert(3, "")
    text = "\n".join(lines) + "\n"
