...
| 权重 | 噪声权重（归一化后相邻差分 MAD 的逆方差，夹在中值的 1/16–16 倍）× 配准质量权重（Σpeak/Σflux×FWHM²/星数，按滤镜中值归一）；QC 特征不参与 | `calibration.py:2218-2305`；`ufwbpp_registration/pipeline.py:324-335` |
...
3. **权重模型**：统一到一处（现在分散在 `calibration.py` 与 `ufwbpp_registration/quality.py`）：`w_i = (T_i²/σ_i²) · FWHM_i^(−2p) · c_i · n_i`，T 来自已有的恒星尺度提示，σ 来自归一化后的噪声估计，p 由优先级决定（深度 0、平衡 1、分辨率 2），c 为置信度，n 为夜间因子。用 ΔQ 校验这个公式确实优于当前的"噪声 × PSF 代理"。
