...
| Undersampled mono with measured dithers | [Drizzle 2×](drizzle.md) | Synthetic and adapter validation; retained real-data gate pending |
...
| OSC/CFA | No executable recipe | Blocked until phase-preserving calibration, registration, Drizzle/Debayer and color reconstruction pass independent validation |
...
- **Drizzle 2× mono** — two-times output scale with an explicit kernel, pixfrac, weight map, rejection mask, and coverage gate; the GUI disables it when its measured evidence is insufficient.
...
- **OSC/CFA** — a blocked contract, not an executable option in this release.
