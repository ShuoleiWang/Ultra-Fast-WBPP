# XISF pixels, global normalization, and LocalNormalization

> **Pre-1.0 boundary:** one retained real mono MasterFlat validates the bounded XISF decode bridge. Global normalization passed one private 38-frame B run but still lacks retained multi-condition acceptance. LocalNormalization has synthetic validation only. Neither is algorithmically equivalent to PixInsight ImageIntegration/LocalNormalization; global normalization is the conservative ordinary-integration default, while LocalNormalization remains opt-in.
...
The production decoder accepts a unique two-dimensional mono/CFA science image stored as an attachment. A PixInsight container may also contain explicitly identified `RejectionMapLow`, `RejectionMapHigh`, `RejectionMap`, `WeightMap`, or `CoverageMap` auxiliary images; these maps are recorded and ignored. Multiple science images, unknown extra images, RGB/multichannel storage, complex samples, inline/embedded data, malformed geometry, attachment escapes, unsupported codecs, and any image or working set above the configured limits fail closed.
...
## Conservative LocalNormalization

Local normalization is opt-in and disabled by default:

```json
{
  "localNormalization": {"enabled": true, "tileSizePixels": 256}
}
```

For each filter, Ultra-Fast WBPP fits `reference ≈ scale(x,y) * target + offset(x,y)` on a coarse registered-coordinate grid. Paired low/background quantiles and residual clipping exclude bright stars and high-surface-brightness structure from coefficient estimation. A shrinkage-regularized scale grid and an offset grid are bilinearly interpolated; no correction feature is smaller than `tileSizePixels`. Ordinary integration consumes locally normalized registered frames. Drizzle applies the same registered-coordinate model to calibrated source pixels before resampling.

Automatic execution requires at least three frames and passes only when the model has enough valid tiles, adequate paired correlation, bounded scale coefficients and spatial span, and measurable residual improvement. Every normalized frame, scale/offset model, parameters, residual metrics, and content digest is written to run evidence. Insufficient or ambiguous models fail closed. Enabling LocalNormalization disables the ordinary global-normalization stage so scale/offset corrections are never applied twice.

This implementation is scientifically conservative but is not algorithmically equivalent to PixInsight LocalNormalization. Extended targets with unusual morphology or strong non-linear transparency changes should be processed with the stage disabled unless the evidence receipt is reviewed.
