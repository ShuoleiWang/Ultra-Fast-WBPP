...
The current strict final backend is a user-installed local Astrometry.net `solve-field` plus app-managed checked indexes. There is no built-in solver and no online upload fallback. ASTAP has an isolated process adapter for diagnostics, but it cannot satisfy the strict final publication gate because the current adapter cannot independently bind its catalog bytes and correspondence table.
